---
tipas: autorius
pavadinimas: 'R. Jablonskytė-Rimantienė'
saltiniai:
  - 'Zenonas Ivinskis, Lietuvos istorija iki Vytauto Didžiojo mirties (1978 m.)'
sukurta: ''
atnaujinta: ''
tags:
  - autorius
---
# R. Jablonskytė-Rimantienė

## Santrauka

Jablonskytė - Rimantienė pateikė duomenų apie seniausias akmens amžiaus stovyklas Lietuvoje^98.

## Teiginiai

<a id="claim-t-30938"></a>
- t-001
  global_id: t-30938
  teiginys: 'R. Jablonskytė-Rimantienė pateikė duomenų apie seniausias akmens amžiaus stovyklas Lietuvoje.'
  sudarymo_pagrindimas: 'Teiginys yra išbaigtas ir tiesiogiai paremtas citatos formuluote.'
  susije_objektai: 'mentioned_author: [[objektai/autoriai/R. Kulikauskienė|R. Kulikauskienė]]; mentioned_place: Lietuva; mentioned_author: [[objektai/autoriai/V. Urbanavičius|V. Urbanavičius]]'
  pagrindžia:
    - c-001
  irodymo_stiprumas: 0.00
  saltinio_vieta: hash=cc28b172e04ab8359d5720ad7e0739a2a6f7bfe4f1bf771cc198cb5b337b1741; match=fallback; occurrences=0
  sprendimo_priezastis: auto
  ryšio_patikimumas: susije_su -> Lietuva: 0.85
  ryšio_patikimumo_lygis: vidutinis
  ryšio_patikimumo_priezastys: owner_before_predicate; single_candidate_target; single_candidate_actor; target_after_predicate; same_sentence_locality
  ryšio_sprendimo_taisykle: rule_plain_mention
  ryšio_subjekto_parinkimas: R. Jablonskytė-Rimantienė: owner_note_path, author, gap=0
  ryšio_targeto_parinkimas: Lietuva: mention_match, place, gap=83
  ryšio_paaiskinimas: Ryšys sukurtas taisykle "rule_plain_mention". Subjektas "R. Jablonskytė-Rimantienė" parinktas kaip owner_note_path. Targetas "Lietuva" parinktas kaip mention_match aplink predikatą "mention". Patikimumą lėmė: owner_before_predicate, single_candidate_target, single_candidate_actor, target_after_predicate, same_sentence_locality.
- susijęs iš O drevuejšich kul’turnych oblastjach na territorii Litvy: R. Jablonskytė-Rimantienė straipsnį „O drevuejšich kul’turnych oblastjach na territorii Litvy“ paskelbė 1955 m. žurnalo „Sovetskaja Etnografija“ 3 tome.
## Reikšmingi paminėjimai

- c-001
  santrauka: 'R. Jablonskytė-Rimantienė pateikė duomenų apie seniausias akmens amžiaus stovyklas Lietuvoje.'
  šaltinis: Zenonas Ivinskis, Lietuvos istorija iki Vytauto Didžiojo mirties (1978 m.)
  citata_originali: |
    P. Kulikauskas, R. Kulikauskienė ir A. Tautavi­
    čius išleido knygą _Lietuvos archeologijos bruožai_ (1961). Be jų,
    R. Jablonskytė - Rimantienė pateikė duomenų apie
    seniausias akmens amžiaus stovyklas Lietuvoje^98. O N avickai­
    tė - Kuncienė rašė apie įvairius kapinynus bei pilkapius
    V. Urbanavičius domėjosi degintiniais kapais ir laidosena
    Lietuvoje XIV a.^100.
  citata_rodoma: ''
  teiginio_tipas: faktas
  statusas: verified
  patikimumo_lygis: vidutinis
  patikimumo_saltinis: ai
  pagrindžia:
    - t-001