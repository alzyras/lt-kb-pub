---
tipas: zodyno_irasas
pavadinimas: "Patrankų liejykla"
sukurta: "2026-04-18"
variantai: 
  - "patrankų liejykla"
aliases: 
  - "Patrankų liejykla"
  - "patrankų liejykla"
saltiniai:
  - "[[A. Šapoka (red.), Lietuvos istorija (1936 m.)]]"
zymos:
  - "karinis-terminas"
laikotarpis: "1579 m."
datos: 
  - "1579 m."
periodas: "ankstyvieji_naujieji_laikai"
periodo_grupes: []
amziai: 
  - "XVI"
date_start: "1579"
date_end: "1579"
vaidmenys: []
dalyviai: []
susije_irasai: []
patikimumas: "aukstas"
---
# Patrankų liejykla

## Santrauka
Šaltinyje „Patrankų liejykla“ vartojama taip: Patrankoms lieti skirta dirbtuvė arba gamykla. (t-001)

## Žodis ir formos
- Kanoninis pavadinimas: Patrankų liejykla
- Formos šaltinyje: patrankų liejykla

## Laikotarpis ir datos
- laikotarpis: 1579 m.
- datos: 1579 m.
- periodas: ankstyvieji_naujieji_laikai
- periodo_grupes: 
- amziai: XVI
- date_start: 1579
- date_end: 1579

## Reikšmė iš konteksto
- Patrankoms lieti skirta dirbtuvė arba gamykla. (t-001)

## Vartojimas
Terminas šaltinyje vartojamas šiame kontekste: karo pramonė, ginkluotė. (t-001)

## Teiginiai
- id: t-001
  teiginys: "Šaltinyje „Patrankų liejykla“ vartojama taip: Patrankoms lieti skirta dirbtuvė arba gamykla."
  pagrindžia:
    - q-001
  šaltinis: [[A. Šapoka (red.), Lietuvos istorija (1936 m.)]]
  statusas: patvirtinta

## Reikšmingi paminėjimai
- id: q-001
  santrauka: Vilniuje įsteigta karo reikmenų gamybos dirbtuvė.
  šaltinis: [[A. Šapoka (red.), Lietuvos istorija (1936 m.)]]
  citata_originali: |
    Lietuva noriai
    pasižadėjo duoti 10.000 karių. Tuo būdu surinkus pinigų, prisi-
    samdžius Vengrijoje ir Vokietijoje kareivių, prisipirkus ir prisi-
    gaminus ginklų (tada Vilniuje buvo įsteigta patrankų liejykla),
  citata_rodoma: |
    Lietuva noriai pasižadėjo duoti 10.000 karių. Tuo būdu surinkus pinigų, prisisamdžius Vengrijoje ir Vokietijoje kareivių, prisipirkus ir prisigaminus ginklų (tada Vilniuje buvo įsteigta patrankų liejykla),
  teiginio_tipas: faktas
  patikimumo_lygis: aukstas
  patikimumo_saltinis: ai
  patikimumo_pagrindimas: "Citata tiesiogiai pagrindžia termino vartoseną arba aiškinimą šaltinyje."
  pagrindžia:
    - t-001

## Ryšiai
- Šaltinis: [[A. Šapoka (red.), Lietuvos istorija (1936 m.)]]
