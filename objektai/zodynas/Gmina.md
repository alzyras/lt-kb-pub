---
tipas: zodyno_irasas
pavadinimas: "Gmina"
sukurta: "2026-04-18"
variantai: 
  - "gminose"
aliases: 
  - "Gmina"
  - "gminose"
saltiniai:
  - "[[A. Šapoka (red.), Lietuvos istorija (1936 m.)]]"
zymos:
  - "teisinis-terminas"
laikotarpis: "1864 m."
datos: 
  - "1864 m."
periodas: "naujieji_laikai"
periodo_grupes: []
amziai: 
  - "XIX"
date_start: "1864"
date_end: "1864"
vaidmenys: []
dalyviai: []
susije_irasai: []
patikimumas: "aukstas"
---
# Gmina

## Santrauka
Šaltinyje „Gmina“ vartojama taip: valsčius, vietos administracinis vienetas. (t-001)

## Žodis ir formos
- Kanoninis pavadinimas: Gmina
- Formos šaltinyje: gminose

## Laikotarpis ir datos
- laikotarpis: 1864 m.
- datos: 1864 m.
- periodas: naujieji_laikai
- periodo_grupes: 
- amziai: XIX
- date_start: 1864
- date_end: 1864

## Reikšmė iš konteksto
- valsčius, vietos administracinis vienetas. (t-001)

## Vartojimas
Terminas šaltinyje vartojamas šiame kontekste: vietos savivalda, Užnemunės administracija. (t-001)

## Teiginiai
- id: t-001
  teiginys: "Šaltinyje „Gmina“ vartojama taip: valsčius, vietos administracinis vienetas."
  pagrindžia:
    - q-001
  šaltinis: [[A. Šapoka (red.), Lietuvos istorija (1936 m.)]]
  statusas: patvirtinta

## Reikšmingi paminėjimai
- id: q-001
  santrauka: Užnemunėje valsčiai aiškiai įvardyti kaip gminos.
  šaltinis: [[A. Šapoka (red.), Lietuvos istorija (1936 m.)]]
  citata_originali: |
    Bet antrojo laipsnio savivaldybėse — valsčiuose
    (gminose) — dalyvavo visi apylinkės gyventojai, kurie turėjo
    nors 3 margus žemės.
  citata_rodoma: ""
  teiginio_tipas: faktas
  patikimumo_lygis: aukstas
  patikimumo_saltinis: ai
  patikimumo_pagrindimas: "Citata tiesiogiai pagrindžia termino vartoseną arba aiškinimą šaltinyje."
  pagrindžia:
    - t-001

## Ryšiai
- Šaltinis: [[A. Šapoka (red.), Lietuvos istorija (1936 m.)]]
