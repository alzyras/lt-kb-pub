---
tipas: zodyno_irasas
pavadinimas: Prikeltuvės
sukurta: '2026-07-12'
variantai:
  - 'prikeltuves'
  - 'prikeltuvės'
aliases: []
saltiniai:
  - 'Mūsų tautosaka, t.4 (1931)'
bukle: juodrastis
laikotarpis: ''
datos: []
periodas: ''
periodo_grupes: []
amziai: []
date_start: ''
date_end: ''
vaidmenys: []
dalyviai: []
susije_irasai: []
patikimumas: ''
atnaujinta: '2026-07-12'
tags:
  - socialinis-terminas
  - sąvoka
media_total_count: '0'
media_primary_thumb_url: ''
media_primary_canonical_url: ''
media_primary_directness: ''
media_primary_relation_type: ''
media_primary_json: ''
media_direct_json: |-
  []
media_contextual_json: |-
  []
media_all_json: |-
  []
---
# Prikeltuvės

## Žodis ir formos

Šiame šaltinyje vartojamos formos: `prikeltuves`, `prikeltuvės`.

## Reikšmė iš konteksto

Prikeltuvės šiame šaltinyje yra po vestuvių rengiamos jaunamarčios priėmimo apeigos.

## Vartojimas

Terminas vartojamas vestuvių tęsinio ir ištekėjusios moters apdėjimo kontekste.

## Teiginiai

<a id="claim-t-195630"></a>
- t-001
  global_id: t-195630
  teiginys: "Prikeltuvės šiame šaltinyje yra svočios surengiama apeiga."
  atnaujinta: "2026-07-12 08:46"
  sprendimo_priezastis: "gap::vocabulary"
  teiginio_tipas: "faktas"
  šaltinio_profilis: "šališkumas: unknown; atribucija: optional"
  pagrindžia:
    - c-178032

## Citatos

- id: c-178032
  autorius: "M. Katkus"
  šaltinis: "Mūsų tautosaka, t.4 (1931) — M. Katkus, „Balanos gadynė“"
  puslapiai: "p. 120 (PDF 118)"
  indeksas: "Mūsų tautosaka, t.4 (1931) — M. Katkus, „Balanos gadynė“, p. 120 (PDF 118)."
  citata_originali: |
    Ten palieka jaunoji, anot dai­
    nos, „patalėlį klodama, bernelį globodama“ .
    Rytoj rytą ne jaunoji,
    bet jau jaunamartė apsižiūri, kur esanti, kokiam galui ji „augo, go-
    dėjo, didžiai norėjo už bernužėlio į nevalėlę“ .
    Kaip „anoje šalyje“ piršlys buvo pirmas, taip dabar čia svočia:
    turi suruošti vadinamas „prikeltuves“ .
  citata_rodoma: |
    Kaip „anoje šalyje“ piršlys buvo pirmas, taip dabar čia svočia: turi suruošti vadinamas „prikeltuves“ .
  statusas: verified
  atnaujinta: "2026-07-12 08:46"
  teiginio_tipas: faktas
  patikimumo_lygis: vidutinis
  patikimumo_saltinis: ai
  pagrindzia:
    - t-195630