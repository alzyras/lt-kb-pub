---
tipas: zodyno_irasas
pavadinimas: 'Hinterland'
saltiniai:
  - 'Teodoras Narbutas, Lietuvių tautos istorija, t. 2 (1995 m.)'
sukurta: ''
atnaujinta: ''
tags:
  - sąvoka
media_total_count: '0'
media_primary_thumb_url: ''
media_primary_canonical_url: ''
media_primary_directness: ''
media_primary_relation_type: ''
media_primary_json: ''
media_direct_json: |-
  []
media_contextual_json: |-
  []
media_all_json: |-
  []
---
# Hinterland

## Santrauka

Narbutas Natangijos pavadinimą aiškina vartodamas terminą `Hinterland`. Narbutas `Hinterland` apibrėžia kaip užpakalinį, galinį kraštą.

## Žodis ir formos

- Pagrindinė forma: Hinterland.
- Vartojamos formos: Hinterland.

## Reikšmė iš konteksto

užpakalinis, galinis kraštas.

## Vartojimas

geografinė sąvoka / paaiškinamasis vokiškas terminas.

## Pastabos

čia tai tik Narbuto įterptas aiškinamasis terminas, todėl viešame įraše reikės aiškiai parodyti, kad tai yra jo siūlomas atitikmuo.

## Teiginiai

<a id="claim-t-192219"></a>
- t-001
  global_id: t-192219
  teiginys: "Narbutas Natangijos pavadinimą aiškina vartodamas terminą `Hinterland`."
  pagrindžia:
    - c-175072

<a id="claim-t-192561"></a>
- t-002
  global_id: t-192561
  teiginys: "Narbutas nurodo nuomonę, kad Natangija galėjo reikšti užpakalinį arba galinį kraštą, Hinterland."
  pagrindžia:
    - c-175072
## Reikšmingi paminėjimai

- c-001
  šaltinis: Teodoras Narbutas, Lietuvių tautos istorija, t. 2 (1995 m.)
  citata_originali: |
    Natangija (Natangia). Mokslininkai tyrinėtojai nesu­
    taria dėl šio pavadinimo kilmės; manoma, kad tai reiš­
    kia užpakalinį, galinį kraštą, Hinterland, nes jis plyti už
    anksčiau aprašytų2. Panašiau į tikrovę tai, jog buvusi
    atskirą kartą sudaranti gentis, vadinama natangais.
  citata_rodoma: ''
  statusas: verified
  teiginio_tipas: saltinio_teiginys
  patikimumo_lygis: vidutinis
  patikimumo_saltinis: ai
  pagrindžia:
    - t-001
    - t-002
    - t-004

## Citatos

- id: c-175072
  autorius: "Teodoras Narbutas"
  šaltinis: "Teodoras Narbutas, Lietuvių tautos istorija, t. 2 (1995 m.)"
  puslapiai: "PDF 266"
  indeksas: "Teodoras Narbutas, Lietuvių tautos istorija, t. 2 (1995 m.), PDF 266."
  citata_originali: |
    Natangija (Natangia). Mokslininkai tyrinėtojai nesu­
    taria dėl šio pavadinimo kilmės; manoma, kad tai reiš­
    kia užpakalinį, galinį kraštą, Hinterland, nes jis plyti už
    anksčiau aprašytų2. Panašiau į tikrovę tai, jog buvusi
    atskirą kartą sudaranti gentis, vadinama natangais.
  citata_rodoma: "Mokslininkai tyrinėtojai nesu­\ntaria dėl šio pavadinimo kilmės; manoma, kad tai reiš­\nkia užpakalinį, galinį kraštą, Hinterland, nes jis plyti už\nanksčiau aprašytų2."
  statusas: verified
  teiginio_tipas: faktas
  patikimumo_lygis: vidutinis
  patikimumo_saltinis: ai
  pagrindzia:
    - t-192219
    - t-192561
    - t-004
