---
tipas: zodyno_irasas
pavadinimas: Filosofijos kandidato laipsnis
sukurta: '2026-07-12'
variantai:
  - 'Filosofijos kandidato laipsnį'
aliases: []
saltiniai:
  - 'Mūsų tautosaka, t.4 (1931)'
bukle: juodrastis
laikotarpis: ''
datos: []
periodas: ''
periodo_grupes: []
amziai: []
date_start: ''
date_end: ''
vaidmenys: []
dalyviai: []
susije_irasai: []
patikimumas: ''
atnaujinta: '2026-07-12'
tags:
  - istorinė-sąvoka
  - sąvoka
media_total_count: '0'
media_primary_thumb_url: ''
media_primary_canonical_url: ''
media_primary_directness: ''
media_primary_relation_type: ''
media_primary_json: ''
media_direct_json: |-
  []
media_contextual_json: |-
  []
media_all_json: |-
  []
---
# Filosofijos kandidato laipsnis

## Žodis ir formos

Šiame šaltinyje vartojamos formos: `Filosofijos kandidato laipsnį`.

## Reikšmė iš konteksto

Tai akademinio laipsnio pavadinimas, vartojamas A. R. Niemi biografijoje.

## Vartojimas

Terminas šiame šaltinyje siejamas su 1891 m. gautu akademiniu laipsniu.

## Teiginiai

<a id="claim-t-195457"></a>
- t-001
  global_id: t-195457
  teiginys: "A. R. Niemi 1891 m. gavo filosofijos kandidato laipsnį."
  pagrindžia:
    - c-177878
- susijęs iš [[objektai/zodynas/Filosofijos licenciato laipsnis.md#claim-t-195614|Filosofijos licenciato laipsnis]]: A. R. Niemi 1899 m. gavo fil. licenciato laipsnį.
- susijęs iš [[objektai/zodynas/Filosofijos magistro laipsnis.md#claim-t-195613|Filosofijos magistro laipsnis]]: A. R. Niemi 1894 m. gavo fil. magistro laipsnį.
## Citatos

- id: c-177878
  autorius: "A. Sabaliauskas"
  šaltinis: "Mūsų tautosaka, t.4 (1931) — A. Sabaliauskas, „A. a. prof. A. R. Niemi“"
  puslapiai: "p. 3 (PDF 2)"
  indeksas: "Mūsų tautosaka, t.4 (1931) — A. Sabaliauskas, „A. a. prof. A. R. Niemi“, p. 3 (PDF 2)."
  citata_originali: |
    Aš tai pa­
    brėžiu dėlto, kad suprastume prof. N. būdo vieną žymiausių bruožų,
    būtent, užuojautą visoms nuskriaustoms tautoms ir ypač lietuvių tau­
    tai, kurios priespaudą ir kovas jis savo paties akimis matė.
    1888 m., baigęs liciejų, įstojo į Helsinkų universitetą į istorijos fi­
    lologijos fakultetą.
    Filosofijos kandidato laipsnį gavo 1891 m.,
    fil.
  citata_rodoma: "Filosofijos kandidato laipsnį gavo 1891 m"
  statusas: verified
  teiginio_tipas: faktas
  patikimumo_lygis: vidutinis
  patikimumo_saltinis: ai
  pagrindzia:
    - t-195457
