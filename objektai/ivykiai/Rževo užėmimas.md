---
tipas: ivykis
pavadinimas: 'Rževo užėmimas'
saltiniai:
  - 'į Ą'
datos:
  - '1363 m.'
date_start: '1363'
date_end: ''
sukurta: ''
atnaujinta: ''
amziai:
  - 'XIV'
media_total_count: '0'
media_primary_thumb_url: ''
media_primary_canonical_url: ''
media_primary_directness: ''
media_primary_relation_type: ''
media_primary_json: ''
media_direct_json: |-
  []
media_contextual_json: |-
  []
media_all_json: |-
  []
---
# Rževo užėmimas

## Santrauka

1363 m. Rževas, kurio bajorai ir gyventojai, Narbuto manymu, tikriausiai puldinėjo lietuvių vietininkų valdomus kaimyninius kraštus, buvo jėga užgrobtas ir priverstas pasiduoti.

## Laikotarpis ir datos
Nenurodyta

## Dalyviai ir vaidmenys
Nenurodyta

## Eiga
Nenurodyta

## Rezultatas
Nenurodyta

## Santrauka

1363 m. Rževas, kurio bajorai ir gyventojai, Narbuto manymu, tikriausiai puldinėjo lietuvių vietininkų valdomus kaimyninius kraštus, buvo jėga užgrobtas ir priverstas pasiduoti.

## Citatos

- id: c-189453
  šaltinis: "Lietuvių tautos istorija, t. 5"
  puslapiai: "PDF 161"
  indeksas: "Lietuvių tautos istorija, t. 5, PDF 161."
  citata_originali: |
    Kai didysis kunigaikštis perga­
    lingai priartėjo prie Juodosios jūros, nesutramdomi Rževo ba­
    jorai ir gyventojai, tikriausiai puldinėdami kaimyninius kraštus,
    buvusius lietuvių vietininkų valdžioje, davė progą žygiui į tą
    miestą. Nežinia, kas jam vadovavo, aišku tik, kad tais pačiais,
    1363-iaisiais, metais, Algirdui bebūnant Podolėje, Rževas bu­
    vo jėga užbrobtas ir priverstas pasiduoti1.
  statusas: verified
  atnaujinta: "2026-08-16 22:50"
  teiginio_tipas: faktas
  patikimumo_lygis: vidutinis
  patikimumo_saltinis: ai
  pagrindžia:
    - t-001

## Teiginiai

<a id="claim-t-206838"></a>
- t-001
  teiginys: "1363 m. Rževas, kurio bajorai ir gyventojai, Narbuto manymu, tikriausiai puldinėjo lietuvių vietininkų valdomus kaimyninius kraštus, buvo jėga užgrobtas ir priverstas pasiduoti."
  atnaujinta: "2026-09-13 22:14"
  sprendimo_priezastis: "auto"
  teiginio_tipas: "faktas"
  patikimumo_lygis: "vidutinis"
  patikimumo_saltinis: "ai"
  šaltinio_profilis: "žanras: istorinis_tekstas; perspektyva: istoriografine; šališkumas: medium; atribucija: required_for_interpretation; atribucijos vardas: Teodoras Narbutas"
  vertinimo_atnaujinta: "2026-08-12T22:29:09Z"
  pagrindžia:
    - c-189453

