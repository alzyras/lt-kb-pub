---
tipas: ivykis
pavadinimas: 'Lietuvių ir rusų pagalba Konradui, Čersko kunigaikščiui (1285 m.)'
variantai:
  - 'Lietuvių ir rusų pagalba Konradui, Čersko kunigaikščiui'
aliases:
  - 'Lietuvių ir rusų pagalba Konradui, Čersko kunigaikščiui'
saltiniai:
  - 'Teodoras Narbutas, Lietuvių tautos istorija, t. 4 (1997 m.)'
datos:
  - '1285 m.'
date_start: '1285'
date_end: ''
sukurta: ''
atnaujinta: ''
amziai:
  - 'XIII'
media_total_count: '0'
media_primary_thumb_url: ''
media_primary_canonical_url: ''
media_primary_directness: ''
media_primary_relation_type: ''
media_primary_json: ''
media_direct_json: |-
  []
media_contextual_json: |-
  []
media_all_json: |-
  []
---
# Lietuvių ir rusų pagalba Konradui, Čersko kunigaikščiui (1285 m.)

## Santrauka

Konrado į kovas atvesti lietuvių ir rusų pulkai užėmė miestą, išžudė įgulą, paėmė belaisvių ir, viską išplėšę, paliko miestą Konradui.

## Laikotarpis ir datos
Nenurodyta

## Dalyviai ir vaidmenys
Nenurodyta

## Eiga
Nenurodyta

## Rezultatas
Nenurodyta

## Teiginiai

<a id="claim-t-205989"></a>
- t-001
  teiginys: "Konrado į kovas atvesti lietuvių ir rusų pulkai užėmė miestą, išžudė įgulą, paėmė belaisvių ir, viską išplėšę, paliko miestą Konradui."
  atnaujinta: "2026-08-12 08:57"
  sprendimo_priezastis: "auto"
  teiginio_tipas: "faktas"
  patikimumo_lygis: "vidutinis"
  patikimumo_saltinis: "ai"
  šaltinio_profilis: "žanras: istorinis_tekstas; perspektyva: istoriografine; šališkumas: medium; atribucija: required_for_interpretation; atribucijos vardas: Teodoras Narbutas"
  vertinimo_atnaujinta: "2026-08-12T03:00:30Z"
  pagrindžia:
    - c-188604

## Santrauka

Konrado į kovas atvesti lietuvių ir rusų pulkai užėmė miestą, išžudė įgulą, paėmė belaisvių ir, viską išplėšę, paliko miestą Konradui.

## Citatos

- id: c-188604
  šaltinis: "Teodoras Narbutas, Lietuvių tautos istorija, t. 4 (1997 m.) — Lietuvių tautos istorija, t. 4"
  puslapiai: "PDF 280"
  indeksas: "Teodoras Narbutas, Lietuvių tautos istorija, t. 4 (1997 m.) — Lietuvių tautos istorija, t. 4, PDF 280."
  citata_originali: |
    Užkariavę jį, išžudė įgulą, paėmė belais­
    vių, sugrobė viską, ką mieste galėjo rasti, ir taip nu­
    siaubę paliko Konradui. Toliau, įžengę į Sochačevo žemę,
    sugriovė sostinę, persikėlė per Vyslą, grįždami dar Plockui
    pagrasino.
  statusas: verified
  atnaujinta: "2026-08-12 08:57"
  teiginio_tipas: faktas
  patikimumo_lygis: vidutinis
  patikimumo_saltinis: ai
  pagrindzia:
    - t-001
