---
tipas: ivykis
pavadinimas: 'Kryžiuočių žygis į Gardiną (1328 m.)'
variantai:
  - 'Kryžiuočių žygis į Gardiną'
aliases:
  - 'Kryžiuočių žygis į Gardiną'
saltiniai:
  - 'Teodoras Narbutas, Lietuvių tautos istorija, t. 4 (1997 m.)'
sukurta: ''
atnaujinta: ''
media_total_count: '0'
media_primary_thumb_url: ''
media_primary_canonical_url: ''
media_primary_directness: ''
media_primary_relation_type: ''
media_primary_json: ''
media_direct_json: |-
  []
media_contextual_json: |-
  []
media_all_json: |-
  []
---
# Kryžiuočių žygis į Gardiną (1328 m.)

## Santrauka

Kryžiuočių žygio į Gardiną metu šešiasdešimt riterių su kariuomene patraukė Gardino link, tačiau žvalgai pranešė apie lietuvių pasirengimą gynybai. Teodoro Narbuto aprašymu, kryžiuočiai su visomis pajėgomis užpuolė Gardino apylinkes ir šešių mylių ruože jas nuniokojo.

## Laikotarpis ir datos
Nenurodyta

## Dalyviai ir vaidmenys
Nenurodyta

## Eiga
Nenurodyta

## Rezultatas
Nenurodyta

## Teiginiai

<a id="claim-t-205980"></a>
- t-001
  teiginys: "Kryžiuočių žygio į Gardiną metu šešiasdešimt riterių su kariuomene patraukė Gardino link, tačiau žvalgai pranešė apie lietuvių pasirengimą gynybai."
  atnaujinta: "2026-08-12 08:57"
  sprendimo_priezastis: "auto"
  teiginio_tipas: "faktas"
  patikimumo_lygis: "vidutinis"
  patikimumo_saltinis: "ai"
  šaltinio_profilis: "žanras: istorinis_tekstas; perspektyva: istoriografine; šališkumas: medium; atribucija: required_for_interpretation; atribucijos vardas: Teodoras Narbutas"
  vertinimo_atnaujinta: "2026-08-12T03:00:30Z"
  pagrindžia:
    - c-188595

<a id="claim-t-205981"></a>
- t-002
  teiginys: "Teodoro Narbuto aprašymu, kryžiuočiai su visomis pajėgomis užpuolė Gardino apylinkes ir šešių mylių ruože jas nuniokojo."
  atnaujinta: "2026-08-12 08:57"
  sprendimo_priezastis: "auto"
  teiginio_tipas: "faktas"
  patikimumo_lygis: "vidutinis"
  patikimumo_saltinis: "ai"
  šaltinio_profilis: "žanras: istorinis_tekstas; perspektyva: istoriografine; šališkumas: medium; atribucija: required_for_interpretation; atribucijos vardas: Teodoras Narbutas"
  vertinimo_atnaujinta: "2026-08-12T03:00:30Z"
  pagrindžia:
    - c-188596

## Santrauka

Kryžiuočių žygio į Gardiną metu šešiasdešimt riterių su kariuomene patraukė Gardino link, tačiau žvalgai pranešė apie lietuvių pasirengimą gynybai. Teodoro Narbuto aprašymu, kryžiuočiai su visomis pajėgomis užpuolė Gardino apylinkes ir šešių mylių ruože jas nuniokojo.

## Citatos

- id: c-188595
  šaltinis: "Teodoras Narbutas, Lietuvių tautos istorija, t. 4 (1997 m.) — Lietuvių tautos istorija, t. 4"
  puslapiai: "PDF 449"
  indeksas: "Teodoras Narbutas, Lietuvių tautos istorija, t. 4 (1997 m.) — Lietuvių tautos istorija, t. 4, PDF 449."
  citata_originali: |
    Žodžiu, buvo nutarta vėl pradėti kovas.
    Šešiasdešimt riterių nemažos kariuomenės priešakyje pa­
    traukė Gardino link, kur tikėjosi neblogos medžioklės.
    Tačiau pakeliui žvalgai jiems pranešė, kad lietuviai, su­
    žinoję apie šį žygį, gerai pasirengė gynybai.
  citata_rodoma: "Šešiasdešimt riterių nemažos kariuomenės priešakyje pa­\ntraukė Gardino link, kur tikėjosi neblogos medžioklės. \nTačiau pakeliui žvalgai jiems pranešė, kad lietuviai, su­\nžinoję apie šį žygį, gerai pasirengė gynybai."
  statusas: verified
  atnaujinta: "2026-08-12 08:57"
  teiginio_tipas: faktas
  patikimumo_lygis: vidutinis
  patikimumo_saltinis: ai
  pagrindzia:
    - t-001

- id: c-188596
  šaltinis: "Teodoras Narbutas, Lietuvių tautos istorija, t. 4 (1997 m.) — Lietuvių tautos istorija, t. 4"
  puslapiai: "PDF 449"
  indeksas: "Teodoras Narbutas, Lietuvių tautos istorija, t. 4 (1997 m.) — Lietuvių tautos istorija, t. 4, PDF 449."
  citata_originali: |
    Dėl to prie­
    šakinis būrys, turintis keturis šimtus žirgų, jau esantis
    Gardino paviete, rytojaus dieną grįžo į pagrindinę ka­
    riuomenę; tuo atsitraukimu apgavo lietuvius, apsistoju­
    sius stovykloje ties Gardinu; šie, gavę žinią apie priešo
    atsitraukimą, išsiskirstė kas sau. Vos tiktai taip atsitiko,
    kryžiuočiai sugrįžo ir su visomis pajėgomis užpuolė Gar­
    dino apylinkes, kurias per šešias mylias aplinkui nunio­
    kojo. Kai kurie iš tenykščių bajorų, tikriausiai kilusių iš
    senųjų Prūsijos išeivių, laisva valia prisidėjo prie kry­
    žiuočių, iš viso devynios šeimynos — devyniasdešimt ke­
    turi asmenys — išėjo į Prūsiją ir pasikrikštijo1.
  citata_rodoma: "Vos tiktai taip atsitiko, \nkryžiuočiai sugrįžo ir su visomis pajėgomis užpuolė Gar­\ndino apylinkes, kurias per šešias mylias aplinkui nunio­\nkojo. Kai kurie iš tenykščių bajorų, tikriausiai kilusių iš \nsenųjų Prūsijos išeivių, laisva valia prisidėjo prie kry­\nžiuočių, iš viso devynios šeimynos — devyniasdešimt ke­\nturi asmenys — išėjo į Prūsiją ir pasikrikštijo1."
  statusas: verified
  atnaujinta: "2026-08-12 08:57"
  teiginio_tipas: faktas
  patikimumo_lygis: vidutinis
  patikimumo_saltinis: ai
  pagrindzia:
    - t-002
