---
tipas: ivykis
pavadinimas: "Gyventojų išlaidų normuojamasis įstatymas (lex sumptuaria) (1776 m.)"
sukurta: "2026-04-18"
variantai: []
aliases: []
saltiniai: 
  - "[[A. Šapoka (red.), Lietuvos istorija (1936 m.)]]"
zymos: 
  - "seimas"
  - "politinis-sprendimas"
laikotarpis: "1776 m."
datos: 
  - "1776 m."
periodas: ""
periodo_grupes: []
amziai: 
  - "XVIII"
date_start: "1776"
date_end: "1776"
vaidmenys: []
dalyviai: []
susije_irasai: []
patikimumas: "aukstas"
---
# Gyventojų išlaidų normuojamasis įstatymas (lex sumptuaria) (1776 m.)

## Santrauka

Siekiant sumažinti importą ir paremti savąją pramonę, seimas priėmė gyventojų išlaidų normuojamąjį įstatymą. (t-001)

## Laikotarpis ir datos

- laikotarpis: 1776 m. (t-001)
- datos: 1776 m. (t-001)
- amziai: XVIII (t-001)
- date_start/date_end: 1776 / 1776 (t-001)

## Teiginiai

- id: t-001
  teiginys: "Siekiant sumažinti importą ir paremti savąją pramonę, seimas priėmė gyventojų išlaidų normuojamąjį įstatymą."
  pagrindžia:
    - q-001
  šaltinis: [[A. Šapoka (red.), Lietuvos istorija (1936 m.)]]
  statusas: patvirtinta

## Reikšmingi paminėjimai

- id: q-001
  santrauka: Siekiant sumažinti importą ir paremti savąją pramonę, seimas priėmė gyventojų išlaidų normuojamąjį įstatymą.
  šaltinis: [[A. Šapoka (red.), Lietuvos istorija (1936 m.)]]
  citata_originali: |
    Tad, norėdamas sumažinti im-
    portą ir paremti savąją pramonę, 1776 m. seimas išleido gyventojų
    išlaidų normuojamąjį įstatymą **_(lex sumptuaria)._** Juo visiems mies-
    tiečiams ir bajorijai buvo uždrausta taisytis užsienio medžiagomis,
    ponams buvo įsakyta savo tarnus taisyti kuo pigiausiai, o visų vai-
    vadijų bajorijai įsakyta nešioti tam tikros spalvos vietinės medžiagos
    mundierius.
  citata_rodoma: |
    Tad, norėdamas sumažinti importą ir paremti savąją pramonę, 1776 m. seimas išleido gyventojų išlaidų normuojamąjį įstatymą (lex sumptuaria). Juo visiems miestiečiams ir bajorijai buvo uždrausta taisytis užsienio medžiagomis, ponams buvo įsakyta savo tarnus taisyti kuo pigiausiai, o visų vaivadijų bajorijai įsakyta nešioti tam tikros spalvos vietinės medžiagos mundierius.
  teiginio_tipas: faktas
  patikimumo_lygis: aukstas
  patikimumo_saltinis: ai
  patikimumo_pagrindimas: "Citata tiesiogiai pagrindžia teiginį šaltinio tekste."
  pagrindžia:
    - t-001
