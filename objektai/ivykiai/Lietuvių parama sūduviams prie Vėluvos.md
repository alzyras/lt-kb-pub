---
tipas: ivykis
pavadinimas: 'Lietuvių parama sūduviams prie Vėluvos'
saltiniai:
  - 'Teodoras Narbutas, Lietuvių tautos istorija, t. 4 (1997 m.)'
sukurta: ''
atnaujinta: ''
media_total_count: '0'
media_primary_thumb_url: ''
media_primary_canonical_url: ''
media_primary_directness: ''
media_primary_relation_type: ''
media_primary_json: ''
media_direct_json: |-
  []
media_contextual_json: |-
  []
media_all_json: |-
  []
---
# Lietuvių parama sūduviams prie Vėluvos

## Santrauka

Sūduviams į pagalbą pasiųsta lietuvių kariauna apgulė Vėluvos pilį, tačiau vokiečiai išgelbėjo Vėluvos pilį, užmušę lietuvių kariaunos vadą.

## Laikotarpis ir datos
Nenurodyta

## Dalyviai ir vaidmenys
Nenurodyta

## Eiga
Nenurodyta

## Rezultatas
Nenurodyta

## Santrauka

Sūduviams į pagalbą pasiųsta lietuvių kariauna apgulė Vėluvos pilį, tačiau vokiečiai išgelbėjo Vėluvos pilį, užmušę lietuvių kariaunos vadą.

## Citatos

- id: c-188605
  šaltinis: "Teodoras Narbutas, Lietuvių tautos istorija, t. 4 (1997 m.) — Lietuvių tautos istorija, t. 4"
  puslapiai: "PDF 206"
  indeksas: "Teodoras Narbutas, Lietuvių tautos istorija, t. 4 (1997 m.) — Lietuvių tautos istorija, t. 4, PDF 206."
  citata_originali: |
    Sūduviai griebėsi ginklo, kad išvarytų vokiečius iš
    Sambijos, į kurią jiems nusiuntė pagalbą didysis kuni­
    gaikštis. Vėluvos pilis buvo apgulta, ir apsupusieji ėmė­
    si savo darbo su didžiu įkarščiu, nuovoka bei ištverme.
    Pilį išgelbėjo tik vokiečiai, užmušę iš svaidomosios ma­
    šinos paleistu akmeniu lietuvių kariaunos vadą.
  statusas: verified
  atnaujinta: "2026-08-12 08:57"
  teiginio_tipas: faktas
  patikimumo_lygis: vidutinis
  patikimumo_saltinis: ai
  pagrindžia:
    - t-001

## Teiginiai

<a id="claim-t-205990"></a>
- t-001
  teiginys: "Sūduviams į pagalbą pasiųsta lietuvių kariauna apgulė Vėluvos pilį, tačiau vokiečiai išgelbėjo Vėluvos pilį, užmušę lietuvių kariaunos vadą."
  atnaujinta: "2026-09-13 22:14"
  sprendimo_priezastis: "auto"
  teiginio_tipas: "faktas"
  patikimumo_lygis: "vidutinis"
  patikimumo_saltinis: "ai"
  šaltinio_profilis: "žanras: istorinis_tekstas; perspektyva: istoriografine; šališkumas: medium; atribucija: required_for_interpretation; atribucijos vardas: Teodoras Narbutas"
  vertinimo_atnaujinta: "2026-08-12T03:00:30Z"
  pagrindžia:
    - c-188605

