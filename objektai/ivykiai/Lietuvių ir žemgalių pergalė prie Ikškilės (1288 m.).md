---
tipas: ivykis
pavadinimas: 'Lietuvių ir žemgalių pergalė prie Ikškilės (1288 m.)'
variantai:
  - 'Lietuvių ir žemgalių pergalė prie Ikškilės'
aliases:
  - 'Lietuvių ir žemgalių pergalė prie Ikškilės'
saltiniai:
  - 'Teodoras Narbutas, Lietuvių tautos istorija, t. 4 (1997 m.)'
datos:
  - '1288 m.'
date_start: '1288'
date_end: ''
sukurta: ''
atnaujinta: ''
amziai:
  - 'XIII'
media_total_count: '0'
media_primary_thumb_url: ''
media_primary_canonical_url: ''
media_primary_directness: ''
media_primary_relation_type: ''
media_primary_json: ''
media_direct_json: |-
  []
media_contextual_json: |-
  []
media_all_json: |-
  []
---
# Lietuvių ir žemgalių pergalė prie Ikškilės (1288 m.)

## Santrauka

1288 m. prie Ikškilės žemgaliai, remiami Lietuvos didžiojo kunigaikščio pasiųstos kariaunos, laimėjo prieš kryžiuočius, o kryžiuočių magistras žuvo.

## Laikotarpis ir datos
Nenurodyta

## Dalyviai ir vaidmenys
Nenurodyta

## Eiga
Nenurodyta

## Rezultatas
Nenurodyta

## Teiginiai

<a id="claim-t-205908"></a>
- t-001
  teiginys: "1288 m. prie Ikškilės žemgaliai, remiami Lietuvos didžiojo kunigaikščio pasiųstos kariaunos, laimėjo prieš kryžiuočius, o kryžiuočių magistras žuvo."
  atnaujinta: "2026-08-12 08:57"
  sprendimo_priezastis: "auto"
  teiginio_tipas: "faktas"
  patikimumo_lygis: "vidutinis"
  patikimumo_saltinis: "ai"
  šaltinio_profilis: "žanras: istorinis_tekstas; perspektyva: istoriografine; šališkumas: medium; atribucija: required_for_interpretation; atribucijos vardas: Teodoras Narbutas"
  temporaliniai_duomenys: "įvykio data: 1288 m."
  temporalinis_paaiskinimas: "Ši data taikoma teiginyje minimai reikšmei „įvykio data“, o ne visam objekto laikotarpiui."
  temporalinis_llm_pakomentavimas: "Pavadinimo fragmentas paverstas išsamiu sakiniu, įvardijant žemgalius, Lietuvos paramą ir mūšio baigtį. Nepapildyti šaltinyje nenurodyti mūšio dalyviai ar motyvai."
  vertinimo_atnaujinta: "2026-08-12T03:00:30Z"
  pagrindžia:
    - c-188523

## Santrauka

1288 m. prie Ikškilės žemgaliai, remiami Lietuvos didžiojo kunigaikščio pasiųstos kariaunos, laimėjo prieš kryžiuočius, o kryžiuočių magistras žuvo.

## Citatos

- id: c-188523
  šaltinis: "Teodoras Narbutas, Lietuvių tautos istorija, t. 4 (1997 m.) — Lietuvių tautos istorija, t. 4"
  puslapiai: "PDF 283"
  indeksas: "Teodoras Narbutas, Lietuvių tautos istorija, t. 4 (1997 m.) — Lietuvių tautos istorija, t. 4, PDF 283."
  citata_originali: |
    1288 m. Pergalė prieš kryžiuočius. Magistras užmuštas.
    Kitoje valstybės pusėje žemgaliai nesiliovė vaidijęsi su
    kryžiuočiais. Liejosi kraujo upeliai nuo atvertėjų rankos
    žuvusių, juk jie, vaikydamiesi grobio, naikino žmones, kur
    tik užėję. Kad ir kaip ten buvo, bet truputis sėkmės, ne
    kartą lydėjusios žemgalius, lėmė jiems pergalės laurus.
    Tarpais patys puldavo vokiečių gyvenvietes, netgi Rygos
    tvirtovės mūrus pasiekę, kerštaudami niokodavo. Tose
    kovose žemgaliai turėjo prašyti pagalbos didįjį Lietuvos
    kunigaikštį, kuris 1288 metų pradžioje tam tikslui nu­
    siuntė jiems nemažą kariauną.
  statusas: verified
  atnaujinta: "2026-08-12 08:57"
  teiginio_tipas: faktas
  patikimumo_lygis: vidutinis
  patikimumo_saltinis: ai
  pagrindžia:
    - t-001
