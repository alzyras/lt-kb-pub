---
tipas: ivykis
pavadinimas: Vytauto priverstinis rusų tikėjimo priėmimas ir krikščioniško tikėjimo viešas atnaujinimas
saltiniai:
  - 'Lietuvos metraštis, Bychovco kronika (1971 m.)'
sukurta: ''
atnaujinta: ''
media_total_count: '0'
media_primary_thumb_url: ''
media_primary_canonical_url: ''
media_primary_directness: ''
media_primary_relation_type: ''
media_primary_json: ''
media_direct_json: '[]'
media_contextual_json: '[]'
media_all_json: '[]'
---
# Vytauto priverstinis rusų tikėjimo priėmimas ir krikščioniško tikėjimo viešas atnaujinimas

## Santrauka

Ir aš prieš savo norą, jų valią vyk dydamas, tariau: aš priėmiau rusų tikėjimą ir pareiš kiau tai viešai žmonėms, o sykiu slaptai laikiausi savo tikėjimo, kurj pirma buvau priėmęs, krikščionių tikė jimo. Ir taip aš ir šiandien laikausi krikščionių tikėjimo.

## Laikotarpis ir datos
Nenurodyta

## Dalyviai ir vaidmenys
Nenurodyta

## Eiga
Nenurodyta

## Rezultatas
Nenurodyta

## Teiginiai

<a id="claim-t-210448"></a>
- t-001
  teiginys: "Ir taip aš ir šiandien laikausi krikščionių tikėjimo."
  atnaujinta: "2026-09-12 19:43"
  sprendimo_priezastis: "auto"
  teiginio_tipas: "faktas"
  patikimumo_lygis: "vidutinis"
  patikimumo_saltinis: "ai"
  šaltinio_profilis: "žanras: kronika; perspektyva: neutrali_arba_neaiski; šališkumas: high; atribucija: required_for_interpretation; atribucijos vardas: Lietuvos metraštis"
  pagrindžia:
    - c-193582

<a id="claim-t-211642"></a>
- t-002
  teiginys: "Ir aš prieš savo norą, jų valią vyk dydamas, tariau: aš priėmiau rusų tikėjimą ir pareiš kiau tai viešai žmonėms, o sykiu slaptai laikiausi savo tikėjimo, kurj pirma buvau priėmęs, krikščionių tikė jimo."
  atnaujinta: "2026-09-12 19:43"
  sprendimo_priezastis: "auto"
  teiginio_tipas: "faktas"
  patikimumo_lygis: "vidutinis"
  patikimumo_saltinis: "ai"
  šaltinio_profilis: "žanras: kronika; perspektyva: neutrali_arba_neaiski; šališkumas: high; atribucija: required_for_interpretation; atribucijos vardas: Lietuvos metraštis"
  pagrindžia:
    - c-193707

## Santrauka

Ir aš prieš savo norą, jų valią vyk dydamas, tariau: aš priėmiau rusų tikėjimą ir pareiš kiau tai viešai žmonėms, o sykiu slaptai laikiausi savo tikėjimo, kurj pirma buvau priėmęs, krikščionių tikė jimo. Ir taip aš ir šiandien laikausi krikščionių tikėjimo.

## Citatos

- id: c-193582
  autorius: "Anoniminis metraštininkas"
  šaltinis: "Lietuvos metraštis, Bychovco kronika (1971 m.)"
  puslapiai: "PDF 175"
  indeksas: "Anoniminis metraštininkas, Lietuvos metraštis, Bychovco kronika (1971 m.), PDF 175."
  citata_originali: |
    Ir kai kunigaikštis Jogaila atsisėdo Krokuvoje ir
    Krokuvos karalystė 5 jį paėmė, tada aš vėl pareiškiau
    savo tikėjimą. Ir taip aš ir šiandien laikausi krikščionių
    tikėjimo.
  statusas: verified
  pagrindimo_rezimas: "ai_supported_context"
  atnaujinta: "2026-09-12 19:43"
  teiginio_tipas: faktas
  patikimumo_lygis: vidutinis
  patikimumo_saltinis: ai
  pagrindzia:
    - t-001

- id: c-193707
  autorius: "Anoniminis metraštininkas"
  šaltinis: "Lietuvos metraštis, Bychovco kronika (1971 m.)"
  puslapiai: "PDF 175"
  indeksas: "Anoniminis metraštininkas, Lietuvos metraštis, Bychovco kronika (1971 m.), PDF 175."
  citata_originali: |
    Tačiau jie vertė ir
    mane priimti rusų tikėjimų, kad pasidaryčiau visų žmo­
    nių nekenčiamas4. Ir aš prieš savo norą, jų valią vyk­
    dydamas, tariau: aš priėmiau rusų tikėjimą ir pareiš­
    kiau tai viešai žmonėms, o sykiu slaptai laikiausi savo
    tikėjimo, kurj pirma buvau priėmęs, krikščionių tikė­
    jimo.
  statusas: verified
  pagrindimo_rezimas: "ai_supported_context"
  atnaujinta: "2026-09-12 19:43"
  teiginio_tipas: faktas
  patikimumo_lygis: vidutinis
  patikimumo_saltinis: ai
  pagrindzia:
    - t-002
