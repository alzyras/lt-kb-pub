---
tipas: ivykis
pavadinimas: 'Kryžėjų žygis į Žemaičius ir Bisenės pilies apgula'
saltiniai:
  - 'Daukantas, Istorija žemaitiška, t.1, 1995'
sukurta: ''
atnaujinta: ''
media_total_count: '0'
media_primary_thumb_url: ''
media_primary_canonical_url: ''
media_primary_directness: ''
media_primary_relation_type: ''
media_primary_json: ''
media_direct_json: |-
  []
media_contextual_json: |-
  []
media_all_json: |-
  []
---
# Kryžėjų žygis į Žemaičius ir Bisenės pilies apgula

## Santrauka

Apgulus pilį, rados kru vina mūša, kaipogi įgulė Bisenės gyniojo švente nybes su neaprašoma kantrybe ir narsybe, vienok lig nesubrėškus dar pergalėjo meldžionys įgulę ir į pilį varu įsilaužė; iškirtę svietą, pačią pilį sude gino, bet ar įgijo tuo pačiu žygiu.

## Laikotarpis ir datos
Nenurodyta

## Dalyviai ir vaidmenys
Nenurodyta

## Eiga
Nenurodyta

## Rezultatas
Nenurodyta

## Teiginiai

<a id="claim-t-201290"></a>
- t-001
  teiginys: "Apgulus pilį, rados kru vina mūša, kaipogi įgulė Bisenės gyniojo švente nybes su neaprašoma kantrybe ir narsybe, vienok lig nesubrėškus dar pergalėjo meldžionys įgulę ir į pilį varu įsilaužė; iškirtę svietą, pačią pilį sude gino, bet ar įgijo tuo pačiu žygiu."
  atnaujinta: "2026-07-16 14:31"
  sprendimo_priezastis: "auto"
  teiginio_tipas: "faktas"
  patikimumo_lygis: "vidutinis"
  patikimumo_saltinis: "ai"
  šaltinio_profilis: "žanras: istorinis_tekstas; perspektyva: istoriografine; šališkumas: medium; atribucija: required_for_interpretation; atribucijos vardas: Daukantas"
  pagrindžia:
    - c-184640

## Citatos

- id: c-184640
  autorius: "Simonas Daukantas"
  šaltinis: "Daukantas, Istorija žemaitiška, t.1, 1995"
  indeksas: "Simonas Daukantas, Daukantas, Istorija žemaitiška, t.1, 1995."
  citata_originali: |
    Bet visų
    šventesnioji vieta buvo pati sala, kurią tuo žygiu
    vylės mistras išnaikinti. Apgulus pilį, rados kru­
    vina mūša, kaipogi įgulė Bisenės gyniojo švente­
    nybes su neaprašoma kantrybe ir narsybe, vienok
    lig nesubrėškus dar pergalėjo meldžionys įgulę ir
    į pilį varu įsilaužė; iškirtę svietą, pačią pilį sude­
    gino, bet ar įgijo tuo pačiu žygiu kryžėjai pačią
    Rūmų salą, raštuose nėra randama. Antra dalis ka­
    riaunos, nuteriojusi plačias apygardas ugnia ir kar­
    dą, nusluoguota gėrybėmis, atėjo pas mistrą, kur­
    sai, nustojęs didžios daugybės karėjų, grįžo namo.
  statusas: verified
  atnaujinta: "2026-07-16 14:31"
  teiginio_tipas: faktas
  patikimumo_lygis: vidutinis
  patikimumo_saltinis: ai
  pagrindzia:
    - t-001
