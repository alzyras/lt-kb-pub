---
tipas: ivykis
pavadinimas: 'Kryžiuočių žygis į Nalšią ir Raseinių pavietą (1322 m.)'
variantai:
  - 'Kryžiuočių žygis į Nalšią ir Raseinių pavietą'
aliases:
  - 'Kryžiuočių žygis į Nalšią ir Raseinių pavietą'
saltiniai:
  - 'Teodoras Narbutas, Lietuvių tautos istorija, t. 4 (1997 m.)'
datos:
  - '1322 m.'
date_start: '1322'
date_end: ''
sukurta: ''
atnaujinta: ''
amziai:
  - 'XIV'
media_total_count: '0'
media_primary_thumb_url: ''
media_primary_canonical_url: ''
media_primary_directness: ''
media_primary_relation_type: ''
media_primary_json: ''
media_direct_json: |-
  []
media_contextual_json: |-
  []
media_all_json: |-
  []
---
# Kryžiuočių žygis į Nalšią ir Raseinių pavietą (1322 m.)

## Santrauka

Teodoro Narbuto aprašymu, 1322 m. žiemą kryžiuočių armijai vadovavo kunigaikštis Bernardas Svidnickis, grafas Geroldizekė ir du grafai iš Julicho bei Vildenbergo. Teodoro Narbuto aprašymu, kitą dieną po vietovės užpuolimo kryžiuočių kariuomenė perėjo Nemuną ir Raseinių paviete per Klogėnų valsčių bei Ariogalos apylinkes viską niokojo.

## Laikotarpis ir datos
Nenurodyta

## Dalyviai ir vaidmenys
Nenurodyta

## Eiga
Nenurodyta

## Rezultatas
Nenurodyta

## Teiginiai

<a id="claim-t-205929"></a>
- t-001
  teiginys: "Teodoro Narbuto aprašymu, 1322 m. žiemą kryžiuočių armijai vadovavo kunigaikštis Bernardas Svidnickis, grafas Geroldizekė ir du grafai iš Julicho bei Vildenbergo."
  atnaujinta: "2026-08-12 08:57"
  sprendimo_priezastis: "auto"
  teiginio_tipas: "faktas"
  patikimumo_lygis: "vidutinis"
  patikimumo_saltinis: "ai"
  šaltinio_profilis: "žanras: istorinis_tekstas; perspektyva: istoriografine; šališkumas: medium; atribucija: required_for_interpretation; atribucijos vardas: Teodoras Narbutas"
  vertinimo_atnaujinta: "2026-08-12T03:00:30Z"
  pagrindžia:
    - c-188544

<a id="claim-t-205930"></a>
- t-002
  teiginys: "Teodoro Narbuto aprašymu, kitą dieną po vietovės užpuolimo kryžiuočių kariuomenė perėjo Nemuną ir Raseinių paviete per Klogėnų valsčių bei Ariogalos apylinkes viską niokojo."
  atnaujinta: "2026-08-12 08:57"
  sprendimo_priezastis: "auto"
  teiginio_tipas: "faktas"
  patikimumo_lygis: "vidutinis"
  patikimumo_saltinis: "ai"
  šaltinio_profilis: "žanras: istorinis_tekstas; perspektyva: istoriografine; šališkumas: medium; atribucija: required_for_interpretation; atribucijos vardas: Teodoras Narbutas"
  vertinimo_atnaujinta: "2026-08-12T03:00:30Z"
  pagrindžia:
    - c-188545

## Santrauka

Teodoro Narbuto aprašymu, 1322 m. žiemą kryžiuočių armijai vadovavo kunigaikštis Bernardas Svidnickis, grafas Geroldizekė ir du grafai iš Julicho bei Vildenbergo. Teodoro Narbuto aprašymu, kitą dieną po vietovės užpuolimo kryžiuočių kariuomenė perėjo Nemuną ir Raseinių paviete per Klogėnų valsčių bei Ariogalos apylinkes viską niokojo.

## Citatos

- id: c-188544
  šaltinis: "Teodoras Narbutas, Lietuvių tautos istorija, t. 4 (1997 m.) — Lietuvių tautos istorija, t. 4"
  puslapiai: "PDF 398"
  indeksas: "Teodoras Narbutas, Lietuvių tautos istorija, t. 4 (1997 m.) — Lietuvių tautos istorija, t. 4, PDF 398."
  citata_originali: |
    Netrukus tos pastangos
    pasiekė savo tikslą, nes, prasidėjus 1322 metų žiemai,
    kryžiuočiai jau susibūrė į armiją, o jiems vadovavo ku­
    nigaikštis Bernardas Svidnickis* 2, grafas Geroldizekė iš
    Svabijos, taip pat dvejetas grafų iš Julicho ir iš Vilden-
    bergo. Be to, daug kitų žymių vokiečių ir čekų didikų
    su tarnais, savo lėšomis surinkusių žmones, ir nemažai
    įvairių luomų žmonių, atvykusių pavieniui. Prūsijos pro­
    vincijos magistras ir didžiojo magistro pavaduotojas Fri-
    derikas Vildenbergas sutelkė visas ordino pajėgas ir va­
    dovavo jeigu ne pačiam gausiausiam, tai pačiam garsiau­
    siam, koks tik vyko Prūsijoje, kryžiaus žygiui.
  statusas: verified
  atnaujinta: "2026-08-12 08:57"
  teiginio_tipas: faktas
  patikimumo_lygis: vidutinis
  patikimumo_saltinis: ai
  pagrindzia:
    - t-001

- id: c-188545
  šaltinis: "Teodoras Narbutas, Lietuvių tautos istorija, t. 4 (1997 m.) — Lietuvių tautos istorija, t. 4"
  puslapiai: "PDF 399"
  indeksas: "Teodoras Narbutas, Lietuvių tautos istorija, t. 4 (1997 m.) — Lietuvių tautos istorija, t. 4, PDF 399."
  citata_originali: |
    Tasai 'krašto užkampis buvo užpultas ir nusiaubtas
    ugnimi bei kalaviju. Vis dėlto šventykla negalėjo savo
    ištaigingumu atitikti to įvaizdžio, koks apie ją buvo tu­
    rėtas, žinoma, iš šnipų pasakojimų, kadangi istorija tyli
    apie tai, kas atsitiko užimant tą vietovę, vis dėlto kaž­
    kokia pilaitė ten buvo užimta ir ugnimi paleista. Rytojaus
    dieną po šio atsitikimo kariuomenė perėjo Nemuną ir įsi­
    brovė į Raseinių pavietą, žygiuodama per Klogėnų vals­
    čių ir Ariogalos apylinkes, pakeliui nepaliko nieko nesu­
    naikinto.
  statusas: verified
  atnaujinta: "2026-08-12 08:57"
  teiginio_tipas: faktas
  patikimumo_lygis: vidutinis
  patikimumo_saltinis: ai
  pagrindzia:
    - t-002
