---
tipas: ivykis
pavadinimas: 'Pirmasis kryžiaus žygis į Lenkiją (1219 m.)'
variantai:
  - 'Pirmasis kryžiaus žygis į Lenkiją'
aliases:
  - 'Pirmasis kryžiaus žygis į Lenkiją'
saltiniai:
  - 'Teodoras Narbutas, Lietuvių tautos istorija, t. 3 (1994 m.)'
datos:
  - '1219 m.'
date_start: '1219'
date_end: ''
sukurta: ''
atnaujinta: ''
amziai:
  - 'XIII'
media_total_count: '0'
media_primary_thumb_url: ''
media_primary_canonical_url: ''
media_primary_directness: ''
media_primary_relation_type: ''
media_primary_json: ''
media_direct_json: |-
  []
media_contextual_json: |-
  []
media_all_json: |-
  []
---
# Pirmasis kryžiaus žygis į Lenkiją (1219 m.)

## Santrauka

Kryžiaus žygis esą paskelbtas ne pavergti ar kamuoti ne laisvėje pagonis, dėl to jų priešinimasis taptų dar įnirlingesnis: kryžiuočiai privalo laikyti save kovotojais, gi nančiais garbingus Išganytojo tikslus, susilaikyti nuo ko kių nors draudžiamų veiksmų ir.

## Laikotarpis ir datos
Nenurodyta

## Dalyviai ir vaidmenys
Nenurodyta

## Eiga
Nenurodyta

## Rezultatas
Nenurodyta

## Reikšmingi paminėjimai
- c-001
  santrauka: 'Kryžiaus žygis esą paskelbtas ne pavergti ar kamuoti ne laisvėje pagonis, dėl to jų priešinimasis taptų dar įnirlingesnis: kryžiuočiai privalo laikyti save kovotojais, gi nančiais garbingus Išganytojo tikslus, susilaikyti nuo ko kių nors draudžiamų veiksmų ir.'
  šaltinis: Teodoras Narbutas, Lietuvių tautos istorija, t. 3 (1994 m.)
  citata_originali: |
    Bet kai į
    žygį leidosi kryžiuočių karinės pajėgos, be abejonės, pa­
    lies Kristijono vadovaujamos, popiežius Honoratas jam
    liepė 1219 metų gegužės 12 ęlieną atskleisti kryžiuočiams
    ir jų vadams tikruosius šventojo sumanymo siekius. Ir
    būtent, kad jie esą pašaukti tik ginti naująją bažnyčią ir
    naujakrikščius, o ne tenkinti žemiškąjį godumo potraukį.
    Kryžiaus žygis esą paskelbtas ne pavergti ar kamuoti ne­
    laisvėje pagonis, dėl to jų priešinimasis taptų dar įnir-
    lingesnis: kryžiuočiai privalo laikyti save kovotojais, gi­
    nančiais garbingus Išganytojo tikslus, susilaikyti nuo ko­
    kių nors draudžiamų veiksmų ir visu kuo paklusti tenykš­
    čio vyskupo valiai ir pamokymams. Дат buvo suteikta
    teisė bausti nusikaltėlius, taikant bažnyčios kanonus1. O
    bendras vadovavimas ir visi reikalai buvo patikėti Gnez-
    no arkivyskupui, pripažinus jį popiežiaus legatu1 2
  citata_rodoma: ''
  teiginio_tipas: faktas
  patikimumo_lygis: vidutinis
  patikimumo_saltinis: ai
  statusas: verified
## Santrauka

Kryžiaus žygis esą paskelbtas ne pavergti ar kamuoti ne laisvėje pagonis, dėl to jų priešinimasis taptų dar įnirlingesnis: kryžiuočiai privalo laikyti save kovotojais, gi nančiais garbingus Išganytojo tikslus, susilaikyti nuo ko kių nors draudžiamų veiksmų ir.

## Citatos

- id: c-187709
  šaltinis: "Lietuvių tautos istorija, t. 3"
  puslapiai: "PDF 292"
  indeksas: "Lietuvių tautos istorija, t. 3, PDF 292."
  citata_originali: |
    Bet kai į
    žygį leidosi kryžiuočių karinės pajėgos, be abejonės, pa­
    lies Kristijono vadovaujamos, popiežius Honoratas jam
    liepė 1219 metų gegužės 12 ęlieną atskleisti kryžiuočiams
    ir jų vadams tikruosius šventojo sumanymo siekius. Ir
    būtent, kad jie esą pašaukti tik ginti naująją bažnyčią ir
    naujakrikščius, o ne tenkinti žemiškąjį godumo potraukį.
    Kryžiaus žygis esą paskelbtas ne pavergti ar kamuoti ne­
    laisvėje pagonis, dėl to jų priešinimasis taptų dar įnir-
    lingesnis: kryžiuočiai privalo laikyti save kovotojais, gi­
    nančiais garbingus Išganytojo tikslus, susilaikyti nuo ko­
    kių nors draudžiamų veiksmų ir visu kuo paklusti tenykš­
    čio vyskupo valiai ir pamokymams. Дат buvo suteikta
    teisė bausti nusikaltėlius, taikant bažnyčios kanonus1. O
    bendras vadovavimas ir visi reikalai buvo patikėti Gnez-
    no arkivyskupui, pripažinus jį popiežiaus legatu1 2.
  citata_rodoma: "Bet kai į\nžygį leidosi kryžiuočių karinės pajėgos, be abejonės, pa­\nlies Kristijono vadovaujamos, popiežius Honoratas jam \nliepė 1219 metų gegužės 12 ęlieną atskleisti kryžiuočiams \nir jų vadams tikruosius šventojo sumanymo siekius. Ir \nbūtent, kad jie esą pašaukti tik ginti naująją bažnyčią ir \nnaujakrikščius, o ne tenkinti žemiškąjį godumo potraukį. \nKryžiaus žygis esą paskelbtas ne pavergti ar kamuoti ne­\nlaisvėje pagonis, dėl to jų priešinimasis taptų dar įnir- \nlingesnis: kryžiuočiai privalo laikyti save kovotojais, gi­\nnančiais garbingus Išganytojo tikslus, susilaikyti nuo ko­\nkių nors draudžiamų veiksmų ir visu kuo paklusti tenykš­\nčio vyskupo valiai ir pamokymams. Дат buvo suteikta \nteisė bausti nusikaltėlius, taikant bažnyčios kanonus1. O \nbendras vadovavimas ir visi reikalai buvo patikėti Gnez- \nno arkivyskupui, pripažinus jį popiežiaus legatu1 2"
  statusas: verified
  atnaujinta: "2026-08-11 18:54"
  teiginio_tipas: faktas
  patikimumo_lygis: vidutinis
  patikimumo_saltinis: ai
  pagrindžia:
    - t-001

## Teiginiai

<a id="claim-t-205320"></a>
- t-001
  teiginys: "Kryžiaus žygis esą paskelbtas ne pavergti ar kamuoti ne laisvėje pagonis, dėl to jų priešinimasis taptų dar įnirlingesnis: kryžiuočiai privalo laikyti save kovotojais, gi nančiais garbingus Išganytojo tikslus, susilaikyti nuo ko kių nors draudžiamų veiksmų ir."
  atnaujinta: "2026-09-13 22:14"
  sprendimo_priezastis: "auto"
  teiginio_tipas: "faktas"
  patikimumo_lygis: "vidutinis"
  patikimumo_saltinis: "ai"
  šaltinio_profilis: "žanras: istorinis_tekstas; perspektyva: istoriografine; šališkumas: medium; atribucija: required_for_interpretation; atribucijos vardas: Teodoras Narbutas"
  vertinimo_atnaujinta: "2026-08-07T20:35:38Z"
  pagrindžia:
    - c-187709

