---
tipas: daiktas
pavadinimas: 'Pastotė'
sukurta: '2026-07-12'
variantai:
  - 'pastotė'
  - 'pastotės'
saltiniai:
  - 'objektai/saltiniai/Mūsų tautosaka, t.4 (1931).md'
atnaujinta: '2026-07-12'
media_total_count: '0'
media_primary_thumb_url: ''
media_primary_canonical_url: ''
media_primary_directness: ''
media_primary_relation_type: ''
media_primary_json: ''
media_direct_json: |-
  []
media_contextual_json: |-
  []
media_all_json: |-
  []
---
# Pastotė

## Santrauka

Vestuvinė kraitvežių ir svodbininkų kelionės priemonė.

## Pavadinimai šaltiniuose

- `pastotė`
- `pastotės`

## Laikotarpis ir datos
Nenurodyta

## Kas tai
Nenurodyta

## Naudojimas
Nenurodyta

## Kontekstas
Nenurodyta

## Teiginiai

<a id="claim-t-195344"></a>
- t-001
  global_id: t-195344
  teiginys: "Vestuvinė kraitvežių ir svodbininkų kelionės priemonė."
  pagrindžia:
    - c-177746
    - c-177748
    - c-177747
- susijęs iš [[objektai/zodynas/Kraičvežiai.md#claim-t-195627|Kraičvežiai]]: Kraičvežiai šiame šaltinyje yra du su pastote atvykstantys kraičio vežėjai.
## Reikšmingi paminėjimai
Nenurodyta

## Citatos

- id: c-177746
  autorius: "M. Katkus"
  šaltinis: "Mūsų tautosaka, t.4 (1931) — M. Katkus, „Balanos gadynė“"
  puslapiai: "p. 117 (PDF 115)"
  indeksas: "Mūsų tautosaka, t.4 (1931) — M. Katkus, „Balanos gadynė“, p. 117 (PDF 115)."
  citata_originali: |
    Toliau jaunieji rado tai, kas buvo pagaminta iš piršlio ir pirš-
    lienės malonės.
    Anaip tėvunainių vestuvėse.
    Trečiadienį
    prieš išvažiuojant į
    „aną šalį“ , pribūna jaunikio parūpinta pastotė su dviem žmonėm —
    tai kraičvežiai.
  citata_rodoma: "Trečiadienį prieš išvažiuojant į „aną šalį“ , pribūna jaunikio parūpinta pastotė su dviem žmonėm — tai kraičvežiai."
  statusas: verified
  teiginio_tipas: faktas
  patikimumo_lygis: vidutinis
  patikimumo_saltinis: ai
  pagrindzia:
    - t-195344

- id: c-177747
  autorius: "M. Katkus"
  šaltinis: "Mūsų tautosaka, t.4 (1931) — M. Katkus, „Balanos gadynė“"
  puslapiai: "p. 119 (PDF 117)"
  indeksas: "Mūsų tautosaka, t.4 (1931) — M. Katkus, „Balanos gadynė“, p. 119 (PDF 117)."
  citata_originali: |
    Visų dėmesys nukreip­
    tas į tai, kas darosi kieme.
    Tenai du žiburiu mirga ir juda — tėvai
    išėjo pasitikti marčią ir, kas dažnai būna,
    pirmą sykį ją pamatyti.
    Grįčioj buvę kaimynai išėjo paimti svodbininkų arklių;
    mat,
    visų
    pastočių ir arklių negalima sutalpinti vienuose namuose.
  citata_rodoma: "Grįčioj buvę kaimynai išėjo paimti svodbininkų arklių; mat, visų pastočių ir arklių negalima sutalpinti vienuose namuose."
  statusas: verified
  teiginio_tipas: faktas
  patikimumo_lygis: vidutinis
  patikimumo_saltinis: ai
  pagrindzia:
    - t-195344

- id: c-177748
  autorius: "M. Katkus"
  šaltinis: "Mūsų tautosaka, t.4 (1931) — M. Katkus, „Balanos gadynė“"
  puslapiai: "p. 117 (PDF 115)"
  indeksas: "Mūsų tautosaka, t.4 (1931) — M. Katkus, „Balanos gadynė“, p. 117 (PDF 115)."
  citata_originali: |
    Anaip tėvunainių vestuvėse.
    Trečiadienį
    prieš išvažiuojant į
    „aną šalį“ , pribūna jaunikio parūpinta pastotė su dviem žmonėm —
    tai kraičvežiai.
    Šiais laikais vienos pastotės negana, reikia dviejų, o
    kartais ir daugiau; aprašomais laikais visada užtekdavo vienos.
  citata_rodoma: "Šiais laikais vienos pastotės negana, reikia dviejų, o kartais ir daugiau; aprašomais laikais visada užtekdavo vienos."
  statusas: verified
  teiginio_tipas: faktas
  patikimumo_lygis: vidutinis
  patikimumo_saltinis: ai
  pagrindzia:
    - t-195344
