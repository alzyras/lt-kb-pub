---
tipas: vieta
pavadinimas: 'Švecas (vieta)'
identifikacija: 'vieta'
variantai:
  - 'Švecas'
aliases:
  - 'Švecas'
saltiniai:
  - 'Daukantas, Istorija žemaitiška, t.1, 1995'
datos:
  - '1245 m.'
date_start: '1245'
date_end: ''
sukurta: ''
atnaujinta: ''
amziai:
  - 'XIII'
media_total_count: '0'
media_primary_thumb_url: ''
media_primary_canonical_url: ''
media_primary_directness: ''
media_primary_relation_type: ''
media_primary_json: ''
media_direct_json: |-
  []
media_contextual_json: |-
  []
media_all_json: |-
  []
---
# Švecas (vieta)

## Santrauka

Lygia dalia antrą seną pilį, vardu Švecas, ties Kulmu ant skardžio Vyslos pastiprino ir įgulę įdė jo, liepdamas nė ten kryžėjų nepraleisti.

## Teiginiai

<a id="claim-t-201181"></a>
- t-001
  teiginys: "Lygia dalia antrą seną pilį, vardu Švecas, ties Kulmu ant skardžio Vyslos pastiprino ir įgulę įdė jo, liepdamas nė ten kryžėjų nepraleisti."
  atnaujinta: "2026-07-16 14:31"
  sprendimo_priezastis: "auto"
  teiginio_tipas: "faktas"
  patikimumo_lygis: "vidutinis"
  patikimumo_saltinis: "ai"
  šaltinio_profilis: "žanras: istorinis_tekstas; perspektyva: istoriografine; šališkumas: medium; atribucija: required_for_interpretation; atribucijos vardas: Daukantas"
  pagrindžia:
    - c-184531

## Citatos

- id: c-184531
  autorius: "Simonas Daukantas"
  šaltinis: "Daukantas, Istorija žemaitiška, t.1, 1995"
  indeksas: "Simonas Daukantas, Daukantas, Istorija žemaitiška, t.1, 1995."
  citata_originali: |
    Regėdamas Sviatopelkas visą kraštą nuteriotą
    praėjusią karę ir matydamas, jog kryžėjai, lindoda-
    mi pilėse Kulmo, Toninės ir Redino, gauna mitalą
    iš Lenkų, Kujavijos ir Kulmijos Vysla pagal, norė­
    damas jiems tą pašalpą užtverti, įkūrė pilį Cantirą
    ant salos to paties vardo ir stiprią įgulę įdėjo, ku­
    rie visas vytines ir valtis, einančias Ysla kryžėjams,
    grobstė ir drevėj o, o irioj us vergė ir žemyn gala­
    vo. Lygia dalia antrą seną pilį, vardu Švecas, ties
    Kulmu ant skardžio Vyslos pastiprino ir įgulę įdė­
    jo, liepdamas nė ten kryžėjų nepraleisti.
    Taip prasidėjo metai 1245 su ta kare.
  statusas: verified
  atnaujinta: "2026-07-16 14:31"
  teiginio_tipas: faktas
  patikimumo_lygis: vidutinis
  patikimumo_saltinis: ai
  pagrindzia:
    - t-001
