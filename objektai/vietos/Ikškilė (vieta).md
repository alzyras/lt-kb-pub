---
tipas: vieta
pavadinimas: 'Ikškilė (vieta)'
identifikacija: 'vieta'
variantai:
  - 'Ikškilė'
aliases:
  - 'Ikškilė'
saltiniai:
  - 'Teodoras Narbutas, Lietuvių tautos istorija, t. 3 (1994 m.)'
datos:
  - '1201 m.'
date_start: '1201'
date_end: ''
sukurta: ''
atnaujinta: ''
amziai:
  - 'XIII'
media_total_count: '0'
media_primary_thumb_url: ''
media_primary_canonical_url: ''
media_primary_directness: ''
media_primary_relation_type: ''
media_primary_json: ''
media_direct_json: |-
  []
media_contextual_json: |-
  []
media_all_json: |-
  []
entity_id: "ent-73165a3aaa3e813f4a99922a"
canonical_name: "Ikškilė (vieta)"
entity_roles: ["place"]
entity_view_role: "place"
entity_aliases: ["Ikškilė","Ikškilė (vieta)"]
sameAs: []
canonical_biography: "Tuo metu vyskupystės katedra iš Ikškilės buvo perkelta į Rygą ir prie jos įkur tas augustinų regulos vienuolynas."
place_authority: true
historical_names: []
---
# Ikškilė (vieta)

## Santrauka

Tuo metu vyskupystės katedra iš Ikškilės buvo perkelta į Rygą ir prie jos įkur tas augustinų regulos vienuolynas.

## Santrauka

Tuo metu vyskupystės katedra iš Ikškilės buvo perkelta į Rygą ir prie jos įkur tas augustinų regulos vienuolynas.

## Citatos

- id: c-187751
  šaltinis: "Lietuvių tautos istorija, t. 3"
  puslapiai: "PDF 271"
  indeksas: "Lietuvių tautos istorija, t. 3, PDF 271."
  citata_originali: |
    1201 m. Bažnyčios organizacija. Tuo metu vyskupystės
    katedra iš Ikškilės buvo perkelta į Rygą ir prie jos įkur­
    tas augustinų regulos vienuolynas.
  statusas: verified
  atnaujinta: "2026-08-11 18:54"
  teiginio_tipas: faktas
  patikimumo_lygis: vidutinis
  patikimumo_saltinis: ai
  pagrindžia:
    - t-001

- id: c-188385
  šaltinis: "Teodoras Narbutas, Lietuvių tautos istorija, t. 4 (1997 m.) — Lietuvių tautos istorija, t. 4"
  puslapiai: "PDF 283"
  indeksas: "Teodoras Narbutas, Lietuvių tautos istorija, t. 4 (1997 m.) — Lietuvių tautos istorija, t. 4, PDF 283."
  citata_originali: |
    Tose
    kovose žemgaliai turėjo prašyti pagalbos didįjį Lietuvos
    kunigaikštį, kuris 1288 metų pradžioje tam tikslui nu­
    siuntė jiems nemažą kariauną. Tatai vyko tuo metu, kai
    magistras Vilkenas Syrborchas buvo sukvietęs į Rygą, į
    kapitulos suvažiavimą, visus komtūrus. Juos užklupo ži­
    nia apie artėjantį priešą, pasiekusį net Ikškilės tvirtovę.
  citata_rodoma: "Juos užklupo ži­\nnia apie artėjantį priešą, pasiekusį net Ikškilės tvirtovę."
  statusas: verified
  atnaujinta: "2026-08-12 08:57"
  teiginio_tipas: faktas
  patikimumo_lygis: vidutinis
  patikimumo_saltinis: ai
  pagrindžia:
    - t-002

## Teiginiai

<a id="claim-t-205251"></a>
- t-001
  teiginys: "Tuo metu vyskupystės katedra iš Ikškilės buvo perkelta į Rygą ir prie jos įkur tas augustinų regulos vienuolynas."
  atnaujinta: "2026-09-13 22:14"
  sprendimo_priezastis: "auto"
  teiginio_tipas: "faktas"
  patikimumo_lygis: "vidutinis"
  patikimumo_saltinis: "ai"
  šaltinio_profilis: "žanras: istorinis_tekstas; perspektyva: istoriografine; šališkumas: medium; atribucija: required_for_interpretation; atribucijos vardas: Teodoras Narbutas"
  temporaliniai_duomenys: "įvykio data: 1201 m."
  temporalinis_paaiskinimas: "Ši data taikoma teiginyje minimai reikšmei „įvykio data“, o ne visam objekto laikotarpiui."
  temporalinis_llm_pakomentavimas: "Ištaisyta OCR klaida ir išlaikyti abu cituojami instituciniai pokyčiai: katedros perkėlimas bei vienuolyno įkūrimas."
  vertinimo_atnaujinta: "2026-08-07T20:35:40Z"
  pagrindžia:
    - c-187751

<a id="claim-t-205770"></a>
- t-002
  teiginys: "Karius užklupo žinia apie artėjantį priešą, kuris buvo pasiekęs net Ikškilės tvirtovę."
  atnaujinta: "2026-09-13 22:14"
  sprendimo_priezastis: "auto"
  teiginio_tipas: "faktas"
  patikimumo_lygis: "vidutinis"
  patikimumo_saltinis: "ai"
  šaltinio_profilis: "žanras: istorinis_tekstas; perspektyva: istoriografine; šališkumas: medium; atribucija: required_for_interpretation; atribucijos vardas: Teodoras Narbutas"
  vertinimo_atnaujinta: "2026-08-12T03:00:31Z"
  pagrindžia:
    - c-188385

