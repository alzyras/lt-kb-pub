---
tipas: vieta
pavadinimas: 'Katinai'
saltiniai:
  - 'Karolis Zikaras (sud.), Žymiausi Lietuvos mūšiai ir karinės operacijos (2013 m.)'
datos:
  - '1919 m.'
date_start: '1919'
date_end: ''
sukurta: ''
atnaujinta: ''
tags:
  - vieta
amziai:
  - 'XX'
---
# Katinai

## Santrauka

BECAS turėjo vykti Katinų | Bendrasis rezervas (vadas - karin. Dešinioji kolona energingu puolimu užėmė plane numatytus Jotvainių, Griniūnų ir Katinų kaimus.

## Teiginiai

<a id="claim-t-41762"></a>
- t-001
  global_id: t-41762
  teiginys: 'Bendrasis rezervas turėjo vykti Katinų kaimo link ir jį užimti.'
  sudarymo_pagrindimas: 'Teiginys aiškiai nusako bendrojo rezervo numatytą veiksmą Katinų atžvilgiu.'
  susije_objektai: 'mentioned_place: Kaimas; mentioned_object: [[objektai/zodynas/bendrasis rezervas|bendrasis rezervas]]; mentioned_object: [[objektai/zodynas/rezervas|rezervas]]'
  temporaliniai_duomenys: 'įvykio data: 1919 m.'
  temporalinis_paaiskinimas: 'Ši data taikoma teiginyje minimai reikšmei „įvykio data“, o ne visam objekto laikotarpiui.'
  temporalinis_llm_pakomentavimas: 'Teiginys aiškiai nusako bendrojo rezervo numatytą veiksmą Katinų atžvilgiu.'
  pagrindžia:
    - c-002
  irodymo_stiprumas: 0.00
  saltinio_vieta: hash=148b590cf9018a1da239db9ce13b4a4d778b96823d68bc2497ea87ca3e321ffc; match=fallback; occurrences=0
  sprendimo_priezastis: auto
  ryšio_patikimumas: susije_su -> Kaimas: 0.85
  ryšio_patikimumo_lygis: vidutinis
  ryšio_patikimumo_priezastys: owner_before_predicate; single_candidate_target; single_candidate_actor; target_after_predicate; same_sentence_locality
  ryšio_sprendimo_taisykle: rule_plain_mention
  ryšio_subjekto_parinkimas: Katinai: owner_note_path, place, gap=0
  ryšio_targeto_parinkimas: Kaimas: mention_match, place, gap=7
  ryšio_paaiskinimas: Ryšys sukurtas taisykle "rule_plain_mention". Subjektas "Katinai" parinktas kaip owner_note_path. Targetas "Kaimas" parinktas kaip mention_match aplink predikatą "mention". Patikimumą lėmė: owner_before_predicate, single_candidate_target, single_candidate_actor, target_after_predicate, same_sentence_locality.

<a id="claim-t-41763"></a>
- t-002
  global_id: t-41763
  teiginys: 'Dešinioji kolona energingu puolimu užėmė Jotvainių, Griniūnų ir Katinų kaimus.'
  sudarymo_pagrindimas: 'Teiginys yra konkretus, gramatiškas ir tiesiogiai paremtas citata.'
  susije_objektai: 'mentioned_place: Griniūnai'
  temporaliniai_duomenys: 'įvykio data: 1919 m.'
  temporalinis_paaiskinimas: 'Ši data taikoma teiginyje minimai reikšmei „įvykio data“, o ne visam objekto laikotarpiui.'
  temporalinis_llm_pakomentavimas: 'Teiginys yra konkretus, gramatiškas ir tiesiogiai paremtas citata.'
  pagrindžia:
    - c-001
  irodymo_stiprumas: 0.00
  saltinio_vieta: hash=89fec3073ea289118d20876dde2ee91a448ebca6e940e8e7789e21f49f708163; match=fallback; occurrences=0
  sprendimo_priezastis: auto
  ryšio_patikimumas: susije_su -> Griniūnai: 0.83
  ryšio_patikimumo_lygis: vidutinis
  ryšio_patikimumo_priezastys: owner_before_predicate; single_candidate_target; single_candidate_actor; same_sentence_locality
  ryšio_sprendimo_taisykle: rule_plain_mention
  ryšio_subjekto_parinkimas: Katinai: owner_note_path, place, gap=0
  ryšio_targeto_parinkimas: Griniūnai: mention_match, place, gap=12
  ryšio_paaiskinimas: Ryšys sukurtas taisykle "rule_plain_mention". Subjektas "Katinai" parinktas kaip owner_note_path. Targetas "Griniūnai" parinktas kaip mention_match aplink predikatą "mention". Patikimumą lėmė: owner_before_predicate, single_candidate_target, single_candidate_actor, same_sentence_locality.
- susijęs iš Griniūnai: Dešinioji kolona energingu puolimu užėmė plane numatytus Jotvainių, Griniūnų ir Katinų kaimus.
- susijęs iš Ašagardas: Narbutas Ašagardą vadina šiaurės Olimpu ir pasakoja, kad jame deivė turėjo Tautų sambūriu vadintus rūmus.
- susijęs iš [[objektai/zodynas/bendrasis rezervas.md#claim-t-38708|bendrasis rezervas]]: Bendrąjį rezervą, vadovaujamą karininko J. Reikalos, sudarė Panevėžio bataliono 2-oji ir 6-oji kuopos bei kiti daliniai.
## Reikšmingi paminėjimai

- c-001
  šaltinis: Karolis Zikaras (sud.), Žymiausi Lietuvos mūšiai ir karinės operacijos (2013 m.)
  citata_originali: |
    Puolimas buvo pradėtas 1919 m. ge-
    gužės 18 d. naktį. Dešinioji kolona ener-
    gingu puolimu užėmė plane numatytus
    Jotvainių, Griniūnų ir Katinų kaimus.
    Viduriniajai vorai po atkaklių kautynių
    pavyko užimti Užkalnius, Ėriškėlius ir
    Barklainius.
  citata_rodoma: ''
  teiginio_tipas: faktas
  statusas: verified
  patikimumo_lygis: vidutinis
  patikimumo_saltinis: ai
  pagrindžia:
    - t-002
- c-002
  šaltinis: Karolis Zikaras (sud.), Žymiausi Lietuvos mūšiai ir karinės operacijos (2013 m.)
  citata_originali: |
    Juos Užkalnių kaimą ir pietinius Barklainius. pulti ir užimti Ėriškių ir Rajūnų kaimus.
    BECAS turėjo vykti Katinų | Bendrasis rezervas (vadas - karin.
    kaimo link ir jį užimti. 1919 m.
  citata_rodoma: ''
  teiginio_tipas: faktas
  statusas: verified
  patikimumo_lygis: vidutinis
  patikimumo_saltinis: ai
  pagrindžia:
    - t-001