---
tipas: asmuo
pavadinimas: 'Stanislovas Narbutas'
saltiniai:
  - 'Albertas Vijūkas-Kojelavičius, Lietuvos istorija (1989 m.)'
sukurta: ''
atnaujinta: ''
---
# Stanislovas Narbutas

## Santrauka

Stanislovas Narbutas Aleksandro buvo pasiųstas į Maskvą parodyti, kad Aleksandras neprieštarauja taikai.

## Teiginiai

<a id="claim-t-186256"></a>
- t-001
  global_id: t-186256
  teiginys: 'Stanislovas Narbutas Aleksandro buvo pasiųstas į Maskvą parodyti, kad Aleksandras neprieštarauja taikai.'
  teiginio_tipas: 'faktas'
  patikimumo_lygis: 'vidutinis'
  patikimumo_saltinis: 'ai'
  sudarymo_pagrindimas: 'Perrašyta, kad sakinio objektas ir faktinis vaidmuo būtų aiškiau susieti su Stanislovu Narbutu.'
  susije_objektai: 'mentioned_place: Maskva; mentioned_person: [[objektai/asmenys/Jonas|Jonas]]; mentioned_person: [[objektai/asmenys/Vladislovas|Vladislovas]]'
  pagrindžia:
    - c-001
  irodymo_stiprumas: 0.00
  saltinio_vieta: 993570-993980; hash=ac7d0096d56b562d5916e5437c42fca66cea574b7aa4ef87b0aa3c85a1320790; match=exact
  sprendimo_priezastis: auto
  ryšio_patikimumas: susije_su -> Maskva: 0.85
  ryšio_patikimumo_lygis: vidutinis
  ryšio_patikimumo_priezastys: owner_before_predicate; single_candidate_target; single_candidate_actor; target_after_predicate; same_sentence_locality
  ryšio_sprendimo_taisykle: rule_plain_mention
  ryšio_subjekto_parinkimas: Stanislovas Narbutas: owner_note_path, person, gap=0
  ryšio_targeto_parinkimas: Maskva: mention_match, place, gap=49
  ryšio_paaiskinimas: Ryšys sukurtas taisykle "rule_plain_mention". Subjektas "Stanislovas Narbutas" parinktas kaip owner_note_path. Targetas "Maskva" parinktas kaip mention_match aplink predikatą "mention". Patikimumą lėmė: owner_before_predicate, single_candidate_target, single_candidate_actor, target_after_predicate, same_sentence_locality.

## Reikšmingi paminėjimai

- c-001
  santrauka: 'Stanislovas Narbutas Aleksandro buvo pasiųstas į Maskvą parodyti, kad Aleksandras neprieštarauja taikai.'
  šaltinis: Albertas Vijūkas-Kojelavičius, Lietuvos istorija (1989 m.)
  citata_originali: |
    Broliai karaliai visaip
    stengėsi padėti Aleksandrui, Vladislovas — per Motie­
    jų Cezelickį, o Jonas — per Aleksandrą Skorutą ragin­
    dami Ivaną sutikti su teisingomis taikos sąlygomis.
    Pats Aleksandras, norėdamas parodyti, jog ir jis ne­
    prieštarauja taikai, nusiuntė į Maskvą Stanislovą Nar­
    butą. Pasiuntinys karštai iš-
    P e i pasiu n tin iu s m ė-
    klojo visas Ivano padarytas
    gina s u sita ik y ti
    skriaudas.
  citata_rodoma: ''
  teiginio_tipas: saltinio_teiginys
  statusas: verified
  pagrindžia:
    - t-001
  irodymo_stiprumas: 0.00
  saltinio_vieta: 993570-993980; hash=ac7d0096d56b562d5916e5437c42fca66cea574b7aa4ef87b0aa3c85a1320790; match=exact
  sprendimo_priezastis: auto
  ryšio_patikimumas: susije_su -> Maskva: 0.85
  ryšio_patikimumo_lygis: vidutinis
  ryšio_patikimumo_priezastys: owner_before_predicate; single_candidate_target; single_candidate_actor; target_after_predicate; same_sentence_locality
  ryšio_sprendimo_taisykle: rule_plain_mention
  ryšio_subjekto_parinkimas: Stanislovas Narbutas: owner_note_path, person, gap=0
  ryšio_targeto_parinkimas: Maskva: mention_match, place, gap=49
  ryšio_paaiskinimas: Ryšys sukurtas taisykle "rule_plain_mention". Subjektas "Stanislovas Narbutas" parinktas kaip owner_note_path. Targetas "Maskva" parinktas kaip mention_match aplink predikatą "mention". Patikimumą lėmė: owner_before_predicate, single_candidate_target, single_candidate_actor, target_after_predicate, same_sentence_locality.
