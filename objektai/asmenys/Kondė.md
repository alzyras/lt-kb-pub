---
tipas: asmuo
pavadinimas: "Kondė"
sukurta: "2026-04-18"
variantai: 
  - "Condė"
  - "Kondei"
aliases: 
  - "Kondė"
saltiniai: 
  - "[[A. Šapoka (red.), Lietuvos istorija (1936 m.)]]"
zymos: []
laikotarpis: ""
datos: []
periodas: ""
periodo_grupes: []
amziai: []
date_start: ""
date_end: ""
vaidmenys: []
dalyviai: []
susije_irasai: []
patikimumas: "vidutinis"
---
# Kondė

## Santrauka
Šaltinyje Kondė apibūdinamas taip: Minimas kaip Prancūzijos kandidatas į Respublikos sostą.

## Variantai
- Condė
- Kondei

## Teiginiai
- id: t-001
  teiginys: "Šaltinyje Kondė apibūdinamas taip: Minimas kaip Prancūzijos kandidatas į Respublikos sostą."
  pagrindžia:
    - q-001
  šaltinis: [[A. Šapoka (red.), Lietuvos istorija (1936 m.)]]
  statusas: patvirtinta

## Reikšmingi paminėjimai
- id: q-001
  santrauka: Minimas kaip Prancūzijos kandidatas į Respublikos sostą.
  šaltinis: [[A. Šapoka (red.), Lietuvos istorija (1936 m.)]]
  citata_originali: |
    Jonas Kazimieras atsisakė nuo sosto, norėdamas
    jį užleisti Prancūzų kandidatui princui Kondei _(Condė)._
    Jo kandidatūrą palaikė ir didieji Lietuvos Lenkijos ponai.
  citata_rodoma: |
    Jonas Kazimieras atsisakė nuo sosto, norėdamas jį užleisti Prancūzų kandidatui princui Kondei (Condė). Jo kandidatūrą palaikė ir didieji Lietuvos Lenkijos ponai.
  teiginio_tipas: faktas
  patikimumo_lygis: vidutinis
  patikimumo_saltinis: ai
  priskyrimo_pagrindas: explicit_variant
  patikimumo_pagrindimas: "Citatoje arba tiesioginiame gretimame kontekste yra asmens vardas, variantas arba stabilus titulas."
  pagrindžia:
    - t-001

## Ryšiai
- Šaltinis: [[A. Šapoka (red.), Lietuvos istorija (1936 m.)]]
