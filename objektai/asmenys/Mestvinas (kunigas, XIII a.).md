---
tipas: asmuo
pavadinimas: 'Mestvinas (kunigas, XIII a.)'
identifikacija: 'kunigas, XIII a.'
variantai:
  - 'Mestvinas'
aliases:
  - 'Mestvinas'
saltiniai:
  - 'Daukantas, Istorija žemaitiška, t.1, 1995'
datos:
  - '1267 m.'
date_start: '1267'
date_end: ''
sukurta: ''
atnaujinta: ''
amziai:
  - 'XIII'
periodo_grupes:
  - 'LDK'
media_total_count: '0'
media_primary_thumb_url: ''
media_primary_canonical_url: ''
media_primary_directness: ''
media_primary_relation_type: ''
media_primary_json: ''
media_direct_json: |-
  []
media_contextual_json: |-
  []
media_all_json: |-
  []
---
# Mestvinas (kunigas, XIII a.)

## Santrauka

Pirmasis jo rūpinimos buvo pasidaryti tarpeju Mestvino, Pamarių kunig., ir su kryžėjais jį sude rinti, kaipogi Mestvimas, apleistas nuo savo brolio, maž tešelpiamas nuo Baimino, kunig.

## Teiginiai

<a id="claim-t-201456"></a>
- t-001
  teiginys: "Pirmasis jo rūpinimos buvo pasidaryti tarpeju Mestvino, Pamarių kunig., ir su kryžėjais jį sude rinti, kaipogi Mestvimas, apleistas nuo savo brolio, maž tešelpiamas nuo Baimino, kunig."
  atnaujinta: "2026-07-16 14:37"
  sprendimo_priezastis: "auto"
  teiginio_tipas: "faktas"
  patikimumo_lygis: "vidutinis"
  patikimumo_saltinis: "ai"
  šaltinio_profilis: "žanras: istorinis_tekstas; perspektyva: istoriografine; šališkumas: medium; atribucija: required_for_interpretation; atribucijos vardas: Daukantas"
  pagrindžia:
    - c-184806

## Citatos

- id: c-184806
  autorius: "Simonas Daukantas"
  šaltinis: "Daukantas, Istorija žemaitiška, t.1, 1995"
  indeksas: "Simonas Daukantas, Daukantas, Istorija žemaitiška, t.1, 1995."
  citata_originali: |
    Tuo-
    mi įsmilintas, Otokaras susigadijo su bavarų vieš­
    pačiu ir atkeliavo rudenį 1267 su kariauna į Parus-
    nį; lig neatėjęs vylės jau didžius daiktus nuveikti,
    mindamas praėjusį savo garbingą žygį, kuriuo ke­
    liomis dienomis Sami ją nukariavo, ko dabar dides-
    niai vylės, turėdamas savo kariaunoj pačius rinkti­
    nius. Pirmasis jo rūpinimos buvo pasidaryti tarpeju
    Mestvino, Pamarių kunig., ir su kryžėjais jį sude­
    rinti, kaipogi Mestvimas, apleistas nuo savo brolio,
    maž tešelpiamas nuo Baimino, kunig.
  statusas: verified
  atnaujinta: "2026-07-16 14:37"
  teiginio_tipas: faktas
  patikimumo_lygis: vidutinis
  patikimumo_saltinis: ai
  pagrindzia:
    - t-001
