---
tipas: grupe
pavadinimas: 'Kalavijonys (grupė)'
identifikacija: 'grupė'
variantai:
  - 'Kalavijonys'
aliases:
  - 'Kalavijonys'
saltiniai:
  - 'Daukantas, Istorija žemaitiška, t.1, 1995'
sukurta: ''
atnaujinta: ''
media_total_count: '0'
media_primary_thumb_url: ''
media_primary_canonical_url: ''
media_primary_directness: ''
media_primary_relation_type: ''
media_primary_json: ''
media_direct_json: |-
  []
media_contextual_json: |-
  []
media_all_json: |-
  []
---
# Kalavijonys (grupė)

## Santrauka

Taip nugalavus Viną, pirmąjį mistrą kalavijonų, tuojau susirinkę kalavijonys iš pilių, arba komtūrų, kuriose gyveno, pakėlė į jo vietą sau mistru brostvį Volkviną (Voląuin), vyrą kantrų, drąsų ir suma nantį, kursai, v.

## Teiginiai

<a id="claim-t-201249"></a>
- t-001
  teiginys: "Taip nugalavus Viną, pirmąjį mistrą kalavijonų, tuojau susirinkę kalavijonys iš pilių, arba komtūrų, kuriose gyveno, pakėlė į jo vietą sau mistru brostvį Volkviną (Voląuin), vyrą kantrų, drąsų ir suma nantį, kursai, v."
  atnaujinta: "2026-07-16 14:31"
  sprendimo_priezastis: "auto"
  teiginio_tipas: "faktas"
  patikimumo_lygis: "vidutinis"
  patikimumo_saltinis: "ai"
  šaltinio_profilis: "žanras: istorinis_tekstas; perspektyva: istoriografine; šališkumas: medium; atribucija: required_for_interpretation; atribucijos vardas: Daukantas"
  pagrindžia:
    - c-184599

## Citatos

- id: c-184599
  šaltinis: "Daukantas, Istorija žemaitiška, t.1, 1995"
  puslapiai: "PDF 212"
  indeksas: "Daukantas, Istorija žemaitiška, t.1, 1995, PDF 212."
  citata_originali: |
    Taip nugalavus Viną, pirmąjį mistrą kalavijonų,
    tuojau susirinkę kalavijonys iš pilių, arba komtūrų,
    kuriose gyveno, pakėlė į jo vietą sau mistru brostvį
    Volkviną (Voląuin), vyrą kantrų, drąsų ir suma­
    nantį, kursai, v. Albertui nesant, visais vokyčiais ir
    perkrikštais rėdė, vienok v. Alberto, kaipo savo
    visų vyriausio valdymiero, klausė.
  statusas: verified
  atnaujinta: "2026-07-16 14:31"
  teiginio_tipas: faktas
  patikimumo_lygis: vidutinis
  patikimumo_saltinis: ai
  pagrindzia:
    - t-001
