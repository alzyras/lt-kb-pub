---
tipas: grupe
pavadinimas: 'Lybiešai (grupė)'
identifikacija: 'grupė'
variantai:
  - 'Lybiešai'
aliases:
  - 'Lybiešai'
saltiniai:
  - 'Daukantas, Istorija žemaitiška, t.1, 1995'
sukurta: ''
atnaujinta: ''
media_total_count: '0'
media_primary_thumb_url: ''
media_primary_canonical_url: ''
media_primary_directness: ''
media_primary_relation_type: ''
media_primary_json: ''
media_direct_json: |-
  []
media_contextual_json: |-
  []
media_all_json: |-
  []
---
# Lybiešai (grupė)

## Santrauka

Nes jog tenai buvo išsidanginę nuo neatmenamų laikų į patį pajūrį tarp tų upių lybiešai, vadinami kuresajar, todėl ta kertelė vadinos kuriesais, arba kuržemiais, vienok visuomet buvo valdžioj žemaičių.

## Teiginiai

<a id="claim-t-201250"></a>
- t-001
  teiginys: "Nes jog tenai buvo išsidanginę nuo neatmenamų laikų į patį pajūrį tarp tų upių lybiešai, vadinami kuresajar, todėl ta kertelė vadinos kuriesais, arba kuržemiais, vienok visuomet buvo valdžioj žemaičių."
  atnaujinta: "2026-07-16 14:31"
  sprendimo_priezastis: "auto"
  teiginio_tipas: "faktas"
  patikimumo_lygis: "vidutinis"
  patikimumo_saltinis: "ai"
  šaltinio_profilis: "žanras: istorinis_tekstas; perspektyva: istoriografine; šališkumas: medium; atribucija: required_for_interpretation; atribucijos vardas: Daukantas"
  pagrindžia:
    - c-184600

## Citatos

- id: c-184600
  šaltinis: "Daukantas, Istorija žemaitiška, t.1, 1995"
  puslapiai: "PDF 130"
  indeksas: "Daukantas, Istorija žemaitiška, t.1, 1995, PDF 130."
  citata_originali: |
    Žemgaliais vadinos žemaičių saulėtekinis galas,
    tai yra gyvenančių paventy ir padaugavy. Nes jog
    tenai buvo išsidanginę nuo neatmenamų laikų į patį
    pajūrį tarp tų upių lybiešai, vadinami kuresajar,
    todėl ta kertelė vadinos kuriesais, arba kuržemiais,
    vienok visuomet buvo valdžioj žemaičių. Atėję pas­
    kui vokyčiai meldžionys 12 amžiuj į Padaugavį vi­
    sų pirma užėmė tuos kuresarus ir pilėse Ventspily
    ir Tukume apsikapėjo ir kraštą patį pramanė pagal
    savo kalbą Kurland, tai yra Kuršo žeme, kurią pas-
    kesniai tuointimpos kariaudami su žemgaliais ir že­
    maičiais kas kartą platino ir didino taip, jog per
    100 metų užėmė ne vien visus Žemgalius, bet dar
    didžią dalį Žemaičių, kaip tuojau regėsim, vienok
    vokyčiai pirmuoju vardu vadino — Kurland, tai
    yra Kuršu, noris gyventojai daugiaus žemgaliai ir
    žemaičiai, ne kuresarai buvo.
  statusas: verified
  atnaujinta: "2026-07-16 14:31"
  teiginio_tipas: faktas
  patikimumo_lygis: vidutinis
  patikimumo_saltinis: ai
  pagrindzia:
    - t-001
