---
tipas: grupe
pavadinimas: 'Totoriai kipčiakai'
saltiniai:
  - 'Teodoras Narbutas, Lietuvių tautos istorija, t. 4 (1997 m.)'
sukurta: ''
atnaujinta: ''
media_total_count: '0'
media_primary_thumb_url: ''
media_primary_canonical_url: ''
media_primary_directness: ''
media_primary_relation_type: ''
media_primary_json: ''
media_direct_json: |-
  []
media_contextual_json: |-
  []
media_all_json: |-
  []
---
# Totoriai kipčiakai

## Santrauka

Trys kunigaikščiai su samdytais totorių pulkais slinko Nemuno link, perėjo upę aukščiau Gardino ir, anot Teodoro Narbuto, taikė į Neries žemupį, kad susijungtų su kryžiuočiais prie Kauno.

## Santrauka

Trys kunigaikščiai su samdytais totorių pulkais slinko Nemuno link, perėjo upę aukščiau Gardino ir, anot Teodoro Narbuto, taikė į Neries žemupį, kad susijungtų su kryžiuočiais prie Kauno.

## Teiginiai

<a id="claim-t-205886"></a>
- t-001
  teiginys: "Trys kunigaikščiai su samdytais totorių pulkais slinko Nemuno link, perėjo upę aukščiau Gardino ir, anot Teodoro Narbuto, taikė į Neries žemupį, kad susijungtų su kryžiuočiais prie Kauno."
  atnaujinta: "2026-08-12 08:57"
  sprendimo_priezastis: "auto"
  teiginio_tipas: "faktas"
  patikimumo_lygis: "vidutinis"
  patikimumo_saltinis: "ai"
  šaltinio_profilis: "žanras: istorinis_tekstas; perspektyva: istoriografine; šališkumas: medium; atribucija: required_for_interpretation; atribucijos vardas: Teodoras Narbutas"
  vertinimo_atnaujinta: "2026-08-12T03:00:29Z"
  pagrindžia:
    - c-188501

## Citatos

- id: c-188501
  šaltinis: "Teodoras Narbutas, Lietuvių tautos istorija, t. 4 (1997 m.) — Lietuvių tautos istorija, t. 4"
  puslapiai: "PDF 377"
  indeksas: "Teodoras Narbutas, Lietuvių tautos istorija, t. 4 (1997 m.) — Lietuvių tautos istorija, t. 4, PDF 377."
  citata_originali: |
    tuomet, kai didysis magistras puolė Žemaitiją. Į tą są­
    jungą įėjo ir Vladimiras Vladimirovičius — Voluinės ku­
    nigaikštis1. Tie trys kunigaikščiai su visomis savo pajė­
    gomis ir samdytais totorių pulkais slinko Nemuno link,
    perėjo tą upę aukščiau Gardino ir taikė į Neries žemupį
    taip, kad aiškiai buvo suvokiamas susijungimo su kry­
    žiuočiais prie Kauno sumanymas.
  citata_rodoma: "Tie trys kunigaikščiai su visomis savo pajė­\ngomis ir samdytais totorių pulkais slinko Nemuno link, \nperėjo tą upę aukščiau Gardino ir taikė į Neries žemupį \ntaip, kad aiškiai buvo suvokiamas susijungimo su kry­\nžiuočiais prie Kauno sumanymas."
  statusas: verified
  atnaujinta: "2026-08-12 08:57"
  teiginio_tipas: faktas
  patikimumo_lygis: vidutinis
  patikimumo_saltinis: ai
  pagrindžia:
    - t-001
