---
tipas: grupe
pavadinimas: 'Lietuvos kariuomenė'
sukurta: '2026-04-25'
variantai: []
aliases: []
saltiniai:
  - 'A. Šapoka (red.), Lietuvos istorija (1936 m.)'
patikimumas: 'vidutinis'
tags:
  - grupe
---
# Lietuvos kariuomenė

## Santrauka

Be to, Napoleono pavesta, ji pradėjo formuoti ir Lietuvos kariuomenę — 4 pėstininkų ir 5 raitelių pulkus.

## Teiginiai
- t-001
  teiginys: 'Be to, Napoleono pavesta, ji pradėjo formuoti ir Lietuvos kariuomenę — 4 pėstininkų ir 5 raitelių pulkus.'
  pagrindžia:
    - c-005
    - c-004
    - c-002
    - c-001

## Reikšmingi paminėjimai
- c-001
  santrauka: 'Be to, Napoleono pavesta, ji pradėjo formuoti ir Lietuvos kariuomenę — 4 pėstininkų ir 5 raitelių pulkus.'
  šaltinis: A. Šapoka (red.), Lietuvos istorija (1936 m.)
  citata_originali: |
    Be to, Napoleono pavesta, ji pradėjo
    formuoti ir Lietuvos kariuomenę — 4 pėstininkų ir 5 raitelių pul-
    kus.
  citata_rodoma: ''
  teiginio_tipas: faktas
  patikimumo_lygis: vidutinis
  patikimumo_saltinis: ai
  patikimumo_pagrindimas: ''
  pagrindžia:
    - t-001
- c-002
  santrauka: Lietuvos kariuomenė papildomas patvirtintas šaltinio paminėjimas.
  šaltinis: Zenonas Ivinskis, Lietuvos istorija iki Vytauto Didžiojo mirties (1978 m.)
  citata_originali: |
    Kiek eilių Vytautas su Jogaila išrikiavo kautynėms, ir ar buvo
    palikti kokie rezervai, žinomos versmės neduoda atsakymo. Deši­
    nysis Vytauto kariuomenės sparnas, po geros valandos smarkaus
    (^20) **DlugO, XIII, 22 p.; CEV, 209 p.**
    (^21) **SRP, III, 437-438 p.**
    (^22) **CEV, 1014 p. ; DlugO, XIII, 51 p.**
    (^23) **DlugO, XIII, 36 p.**
    (^24) **Plg. CEV, 1014, 1032 p.**


    ```
    ŽALGIRIO KAUTYNĖS 341
    ```
    susikovimo, ėmė trauktis link Seewaldo
  citata_rodoma: ""
  teiginio_tipas: faktas
  patikimumo_lygis: aukstas
  patikimumo_saltinis: exact_source_offset
  patikimumo_pagrindimas: "Citata automatiškai pririšta prie exact source offsetų."
  pagrindžia:
    - t-001
    - t-001
- c-004
  santrauka: Lietuvos kariuomenė papildomas patvirtintas šaltinio paminėjimas.
  šaltinis: Zenonas Ivinskis, Lietuvos istorija iki Vytauto Didžiojo mirties (1978 m.)
  citata_originali: |
    Vytauto vadovaujami
    Lietuvos pulkai, kurie paskui save buvo patraukę eilę priešo vėliavų,
    sugrįžo į kovos lauką.
  citata_rodoma: ""
  teiginio_tipas: faktas
  patikimumo_lygis: aukstas
  patikimumo_saltinis: exact_source_offset
  patikimumo_pagrindimas: "Citata automatiškai pririšta prie exact source offsetų."
  pagrindžia:
    - t-001
- c-005
  santrauka: Lietuvos kariuomenė papildomas patvirtintas šaltinio paminėjimas.
  šaltinis: Zenonas Ivinskis, Lietuvos istorija iki Vytauto Didžiojo mirties (1978 m.)
  citata_originali: |
    Vytauto kariuomenė buvo stipriai išretinta ne tik
    Žalgirio kautynėse, bet ir prie Marienburgo prasidėjusių ligų^35.
  citata_rodoma: ""
  teiginio_tipas: faktas
  patikimumo_lygis: aukstas
  patikimumo_saltinis: exact_source_offset
  patikimumo_pagrindimas: "Citata automatiškai pririšta prie exact source offsetų."
  pagrindžia:
    - t-001
    - t-001
    - t-001

## Ryšiai
### Susiję objektai
- [[objektai/grupes/Lietuviai]]
- [[objektai/daiktai/Vėliavos]]
