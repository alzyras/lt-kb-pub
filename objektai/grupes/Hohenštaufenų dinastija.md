---
tipas: grupe
pavadinimas: 'Hohenštaufenų dinastija'
saltiniai:
  - 'Teodoras Narbutas, Lietuvių tautos istorija, t. 3 (1994 m.)'
sukurta: ''
atnaujinta: ''
media_total_count: '0'
media_primary_thumb_url: ''
media_primary_canonical_url: ''
media_primary_directness: ''
media_primary_relation_type: ''
media_primary_json: ''
media_direct_json: |-
  []
media_contextual_json: |-
  []
media_all_json: |-
  []
---
# Hohenštaufenų dinastija

## Santrauka

Narbutas nurodo, kad imperatoriai Fridrikas I ir Henrikas VI iš Hohenštaufenų giminės prisidėjo prie Kryžiuočių ordino iškilimo.

## Santrauka

Narbutas nurodo, kad imperatoriai Fridrikas I ir Henrikas VI iš Hohenštaufenų giminės prisidėjo prie Kryžiuočių ordino iškilimo.

## Teiginiai

<a id="claim-t-205174"></a>
- t-001
  teiginys: "Narbutas nurodo, kad imperatoriai Fridrikas I ir Henrikas VI iš Hohenštaufenų giminės prisidėjo prie Kryžiuočių ordino iškilimo."
  atnaujinta: "2026-08-11 18:54"
  sprendimo_priezastis: "auto"
  teiginio_tipas: "faktas"
  patikimumo_lygis: "vidutinis"
  patikimumo_saltinis: "ai"
  šaltinio_profilis: "žanras: istorinis_tekstas; perspektyva: istoriografine; šališkumas: medium; atribucija: required_for_interpretation; atribucijos vardas: Teodoras Narbutas"
  vertinimo_atnaujinta: "2026-08-07T20:35:37Z"
  pagrindžia:
    - c-187722

## Reikšmingi paminėjimai
- c-001
  santrauka: 'Narbutas nurodo, kad imperatoriai Fridrikas I ir Henrikas VI iš Hohenštaufenų giminės prisidėjo prie Kryžiuočių ordino iškilimo.'
  šaltinis: Teodoras Narbutas, Lietuvių tautos istorija, t. 3 (1994 m.)
  citata_originali: |
    Imperato­
    riai iš Hohenštaufenų giminės — Fridrikas I ir Henrikas
    VI — nemažai prisidėjo prie šio ordino iškilimo.
  citata_rodoma: ''
  teiginio_tipas: faktas
  patikimumo_lygis: vidutinis
  patikimumo_saltinis: ai
  statusas: verified
## Citatos

- id: c-187722
  šaltinis: "Lietuvių tautos istorija, t. 3"
  puslapiai: "PDF 319"
  indeksas: "Lietuvių tautos istorija, t. 3, PDF 319."
  citata_originali: |
    Ta dingstimi popiežius Celestinas II pareikalavo
    iš Joanitų ordino vyresnybės paklusti valiai ir valdžiai
    tų namų prioro, kuris turi būti paskirtas jiems patiems
    jį išrinkus, taip pat nurodė į broliją priimti tik vokiečius.
    Tą potvarkį taip pat patvirtino Hadrijonas IV. Imperato­
    riai iš Hohenštaufenų giminės — Fridrikas I ir Henrikas
    VI — nemažai prisidėjo prie šio ordino iškilimo.
  citata_rodoma: "Imperato­\nriai iš Hohenštaufenų giminės — Fridrikas I ir Henrikas \nVI — nemažai prisidėjo prie šio ordino iškilimo."
  statusas: verified
  atnaujinta: "2026-08-11 18:54"
  teiginio_tipas: faktas
  patikimumo_lygis: vidutinis
  patikimumo_saltinis: ai
  pagrindžia:
    - t-001
