---
tipas: paprotys
pavadinimas: 'Sutarties ir vasaliteto tvirtinimas raštu bei antspaudais (sutartis)'
saltiniai:
  - 'Vytautas Didysis 1350-1430 (1930 m.)'
datos:
  - '1384 m.'
  - '1385 m.'
date_start: '1384'
date_end: '1385'
sukurta: ''
atnaujinta: ''
amziai:
  - 'XIV'
---
# Sutarties ir vasaliteto tvirtinimas raštu bei antspaudais (sutartis)

## Santrauka

XIV a. pabaigos politiniai susitarimai buvo tvirtinami ne vien žodžiu: buvo duodami raštiški pažadai, išrašomi vasaliteto aktai ir prie svarbių dokumentų kabinami valdovų antspaudai. Toks formalizavimas veikė kaip teisėtas politinių įsipareigojimų patvirtinimas.

## Laikotarpis ir datos

1384 m. sausio 30 d.; 1385 m. rugpjūčio 14 d.

## Kas tai

- kategorija: teisinė ir administracinė praktika

## Atlikimas

Susitarimai būdavo įforminami raštu, dokumentuose įrašant įsipareigojimus ir juos sutvirtinant valdovų antspaudais.

## Paskirtis

Formaliai patvirtinti politinius pažadus, vasaliteto ryšį ir sutartinius įsipareigojimus.

## Kontekstas

Praktika minima Vytauto santykiuose su Ordinu ir Jogailos dokumentiniame pažadų patvirtinime po derybų su lenkais ir vengrais.

## Teiginiai

<a id="claim-t-67216"></a>
- t-001
  global_id: t-67216
  teiginys: '1385 m. rugpjūčio 14 d. Jogailos pažadus patvirtinęs dokumentas buvo papildomai sustiprintas Vytauto, Skirgailos, Kaributo ir Lengvenio antspaudais.'
  susije_objektai: 'mentioned_object: [[objektai/daiktai/Antspaudai|Antspaudai]]; mentioned_person: [[objektai/asmenys/Lengvenis|Lengvenis]]; mentioned_group: [[objektai/grupes/Lenkai|Lenkai]]; mentioned_person: [[objektai/asmenys/Jogaila|Jogaila]]; mentioned_person: [[objektai/asmenys/Kaributas|Kaributas]]; mentioned_person: [[objektai/asmenys/Kęstutis|Kęstutis]]'
  temporaliniai_duomenys: 'įvykio data: 1385 m.'
  temporalinis_paaiskinimas: 'Ši data taikoma teiginyje minimai reikšmei „įvykio data“, o ne visam objekto laikotarpiui.'
  pagrindžia:
    - c-001

## Reikšmingi paminėjimai

- c-001
  šaltinis: Vytautas Didysis 1350-1430 (1930 m.)
  citata_originali: |
    Po trumpų tarp abiejų pusių trukusių derybų, 1385 metų
    rugpjūčio mėn. 14 d. toje pačioje pilyje, kur pabaigė savo dienas
    garbingas kunigaikštis Kęstutis ir tokio pat likimo buvo laukęs jo
    sūnus Vytautas, Jogaila dokumentališkai patvirtino visus paža­
    dus, kuriuos jo pasiuntiniai buvo davę lenkams Krokuvoj ir ven­
    grams Budapešte. Prie to dokumento, kuris turėjo fatališkos
    reikšmės Lietuvai, savo antspaudas, be Jogailos, pridėjo dar Vy­
    tautas, Skirgaila, Kaributas ir Lengvenis.
  citata_rodoma: ''
  teiginio_tipas: faktas
  statusas: verified
  pagrindžia:
    - t-001
