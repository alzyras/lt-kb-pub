---
tipas: paprotys
pavadinimas: 'Klojime pasakojamos pasakos ir juokai'
sukurta: ''
atnaujinta: ''
variantai: []
aliases: []
saltiniai:
  - 'Mūsų tautosaka, t.4 (1931)'
laikotarpis: ''
datos: []
periodas: ''
periodo_grupes: []
amziai: []
date_start: ''
date_end: ''
vaidmenys: []
dalyviai: []
susije_irasai: []
patikimumas: ''
tags:
  - paprotys
  - socialinė-praktika
  - švietimas
media_total_count: '0'
media_primary_thumb_url: ''
media_primary_canonical_url: ''
media_primary_directness: ''
media_primary_relation_type: ''
media_primary_json: ''
media_direct_json: |-
  []
media_contextual_json: |-
  []
media_all_json: |-
  []
---
# Klojime pasakojamos pasakos ir juokai

## Santrauka

Kūlimo metu klojime klausytasi pasakų ir juokavimų, o vienas juokingas žodis galėdavo pakelti viso ryto nuotaiką.

## Laikotarpis ir datos
Nenurodyta

## Kas tai
- kategorija: kultūrinė praktika

## Atlikimas
- kas atlieka: klojime susirinkę darbininkai, mergaitės, vaikinai ir klausytojai
- kada atliekama: kūlimo metu, rytais klojime
- kaip atliekama: klausomasi pasakų, juokaujama ir kartojami juokingi žodžiai

## Paskirtis
Nenurodyta

## Kontekstas
Nenurodyta

## Teiginiai

<a id="claim-t-195370"></a>
- t-001
  global_id: t-195370
  teiginys: "Kūlimo metu klojime klausytasi juokavimų ir pasakų."
  atnaujinta: "2026-07-12 02:18"
  sprendimo_priezastis: "final::darbas/prompts/03_extraction/06_extract_customs_notes.md"
  teiginio_tipas: "faktas"
  šaltinio_profilis: "šališkumas: unknown; atribucija: optional"
  pagrindžia:
    - c-177787
    - c-177786

<a id="claim-t-195371"></a>
- t-002
  global_id: t-195371
  teiginys: "Klojimo juokai ir vienas juokingas žodis galėjo išlaikyti gerą nuotaiką keliems rytams."
  atnaujinta: "2026-07-12 02:18"
  sprendimo_priezastis: "final::darbas/prompts/03_extraction/06_extract_customs_notes.md"
  teiginio_tipas: "faktas"
  šaltinio_profilis: "šališkumas: unknown; atribucija: optional"
  pagrindžia:
    - c-177785
    - c-177784

## Reikšmingi paminėjimai
Nenurodyta

## Citatos

- id: c-177784
  autorius: "M. Katkus"
  šaltinis: "Mūsų tautosaka, t.4 (1931) — M. Katkus, „Balanos gadynė“"
  puslapiai: "p. 68 (PDF 66)"
  indeksas: "Mūsų tautosaka, t.4 (1931) — M. Katkus, „Balanos gadynė“, p. 68 (PDF 66)."
  citata_originali: |
    — „Ne, tu sakei prikeltuvė“ .
    Dabar
    vaikis keršija už baravykų gorčių ir šneka apie
    prikeltuves.
    Vieno
    juokingo žodžio užtenka viso ryto nuotaikai pakelti.
  citata_rodoma: |
    Vieno juokingo žodžio užtenka viso ryto nuotaikai pakelti.
  statusas: verified
  atnaujinta: "2026-07-12 02:18"
  teiginio_tipas: faktas
  patikimumo_lygis: vidutinis
  patikimumo_saltinis: ai
  pagrindzia:
    - t-195371
- id: c-177785
  autorius: "M. Katkus"
  šaltinis: "Mūsų tautosaka, t.4 (1931) — M. Katkus, „Balanos gadynė“"
  puslapiai: "p. 68 (PDF 66)"
  indeksas: "Mūsų tautosaka, t.4 (1931) — M. Katkus, „Balanos gadynė“, p. 68 (PDF 66)."
  citata_originali: |
    Pramogos užtekdavo keliems rytams.
    Kitą
    sykį mergaitė pasakė, kad spragilo siautuvė kitur vadinasi „keltuve“ .
    Iš to Juozas pasigavo ir sako: „Žiūrėkite, kas jai galvoje, mat, „prikel-
    tuvė“ .
  citata_rodoma: |
    Pramogos užtekdavo keliems rytams. Kitą sykį mergaitė pasakė, kad spragilo siautuvė kitur vadinasi „keltuvė“ . Iš to Juozas pasigavo ir sako: „Žiūrėkite, kas jai galvoje, mat, „prikeltuvė“ .
  statusas: verified
  atnaujinta: "2026-07-12 02:18"
  teiginio_tipas: faktas
  patikimumo_lygis: vidutinis
  patikimumo_saltinis: ai
  pagrindzia:
    - t-195371
- id: c-177786
  autorius: "M. Katkus"
  šaltinis: "Mūsų tautosaka, t.4 (1931) — M. Katkus, „Balanos gadynė“"
  puslapiai: "p. 68 (PDF 66)"
  indeksas: "Mūsų tautosaka, t.4 (1931) — M. Katkus, „Balanos gadynė“, p. 68 (PDF 66)."
  citata_originali: |
    Šei­
    mininkas į tai: „Tu žiūrėk, kad gegutė tave su duona užkukuotų, kad
    tu terboje duonos turėtum, bus tau geriau, negu pinigai“ .
    Aš klausy­
    damas dėjaus sau į galvą, kad gal duona ir geriau už pinigus.
    Štai pasakos klojime girdėtos.
  citata_rodoma: |
    Štai pasakos klojime girdėtos.
  statusas: verified
  atnaujinta: "2026-07-12 02:18"
  teiginio_tipas: faktas
  patikimumo_lygis: vidutinis
  patikimumo_saltinis: ai
  pagrindzia:
    - t-195370
- id: c-177787
  autorius: "M. Katkus"
  šaltinis: "Mūsų tautosaka, t.4 (1931) — M. Katkus, „Balanos gadynė“"
  puslapiai: "p. 67 (PDF 65)"
  indeksas: "Mūsų tautosaka, t.4 (1931) — M. Katkus, „Balanos gadynė“, p. 67 (PDF 65)."
  citata_originali: |
    Ant papėdės bent pusės masto aukštumo buvo sukrauta ak­
    menų krosnelė su plačia ir aukšta kakta; toje
    krosnelėje
    kūrenosi
    ugnis ir švietė klojimą.
    Malkas vartodavo alksnines ar liepines, nes
    jos nesproginėja ir liepsningai dega.
    Aš kurstydavau, šildydavaus ir
    klausydavau juokavimų.
  citata_rodoma: |
    Aš kurstydavau, šildydavaus ir klausydavau juokavimų.
  statusas: verified
  atnaujinta: "2026-07-12 02:18"
  teiginio_tipas: faktas
  patikimumo_lygis: vidutinis
  patikimumo_saltinis: ai
  pagrindzia:
    - t-195370