---
tipas: paprotys
pavadinimas: "Žydų seimų rinkimasis ir mokesčių rinkėjo rinkimas"
sukurta: "2026-04-18"
variantai: 
  - "XVII amž. įsigalėjo pačių žydų seimai. Seimai išrinkdavo vieną visai Lietuvai žydų mokesčių rinkėją ir gynė žydų reikalus prieš valdžią."
aliases: 
  - "Žydų seimų rinkimasis ir mokesčių rinkėjo rinkimas"
  - "XVII amž. įsigalėjo pačių žydų seimai. Seimai išrinkdavo vieną visai Lietuvai žydų mokesčių rinkėją ir gynė žydų reikalus prieš valdžią."
saltiniai: 
  - "[[A. Šapoka (red.), Lietuvos istorija (1936 m.)]]"
zymos: 
  - "teisinė-praktika"
laikotarpis: "XVII amž."
datos: 
  - "XVII amž."
periodas: ""
periodo_grupes: []
amziai: 
  - "XVII"
date_start: ""
date_end: ""
vaidmenys: []
dalyviai: []
susije_irasai: []
patikimumas: "vidutinis"
---
# Žydų seimų rinkimasis ir mokesčių rinkėjo rinkimas

## Santrauka
Šaltinyje minima praktika „Žydų seimų rinkimasis ir mokesčių rinkėjo rinkimas“ siejama su formuluote: XVII amž. įsigalėjo pačių žydų seimai. Seimai išrinkdavo vieną visai Lietuvai žydų mokesčių rinkėją ir gynė žydų reikalus prieš valdžią.. Šaltinio kontekste nurodoma: kas atlieka: pačių žydų seimai; kada atliekama: XVII amž.. (t-001)

## Pavadinimai šaltiniuose
- XVII amž. įsigalėjo pačių žydų seimai. Seimai išrinkdavo vieną visai Lietuvai žydų mokesčių rinkėją ir gynė žydų reikalus prieš valdžią.

## Laikotarpis ir datos
- laikotarpis: XVII amž. (t-001)
- datos: XVII amž. (t-001)
- amziai: XVII (t-001)

## Kas tai
Šaltinyje minima praktika „Žydų seimų rinkimasis ir mokesčių rinkėjo rinkimas“ siejama su formuluote: XVII amž. įsigalėjo pačių žydų seimai. Seimai išrinkdavo vieną visai Lietuvai žydų mokesčių rinkėją ir gynė žydų reikalus prieš valdžią.. Šaltinio kontekste nurodoma: kas atlieka: pačių žydų seimai; kada atliekama: XVII amž.. (t-001)

## Atlikimas
- kas atlieka: pačių žydų seimai (t-001)
- kada atliekama: XVII amž. (t-001)
- kaip atliekama: XVII amž. įsigalėjo pačių žydų seimai. Seimai išrinkdavo vieną visai Lietuvai žydų mokesčių rinkėją ir gynė žydų reikalus prieš valdžią. (t-001)

## Teiginiai
- id: t-001
  teiginys: "Šaltinyje minima praktika „Žydų seimų rinkimasis ir mokesčių rinkėjo rinkimas“ siejama su formuluote: XVII amž. įsigalėjo pačių žydų seimai. Seimai išrinkdavo vieną visai Lietuvai žydų mokesčių rinkėją ir gynė žydų reikalus prieš valdžią.. Šaltinio kontekste nurodoma: kas atlieka: pačių žydų seimai; kada atliekama: XVII amž.."
  pagrindžia:
    - q-001
  šaltinis: [[A. Šapoka (red.), Lietuvos istorija (1936 m.)]]
  statusas: patvirtinta

## Reikšmingi paminėjimai
- id: q-001
  santrauka: Tekstas aprašo visai Lietuvos žydų bendruomenėms bendrą savivaldos ir atstovavimo praktiką, kai seimai rinkdavo mokesčių rinkėją ir gindavo bendruomenės reikalus prieš valdžią.
  šaltinis: [[A. Šapoka (red.), Lietuvos istorija (1936 m.)]]
  citata_originali: |
    Kai žydų atsirado visame krašte, valdžia atskiroms jų ben-
    druomenėms bandė skirti viršininkus; tačiau XVII amž. įsiga-
    lėjo pačių žydų seimai. Seimai išrinkdavo vieną visai Lietuvai
    žydų mokesčių rinkėją ir gynė žydų reikalus prieš valdžią. Žo-
    džiu, jie buvo visų žydų bendruomenių centras. Tie žydų seimai
    dažniausiai rinkdavosi Brastoje, nes ten buvo seniausioji žydų
    bendruomenė.
  citata_rodoma: |
    Kai žydų atsirado visame krašte, valdžia atskiroms jų bendruomenėms bandė skirti viršininkus; tačiau XVII amž. įsigalėjo pačių žydų seimai. Seimai išrinkdavo vieną visai Lietuvai žydų mokesčių rinkėją ir gynė žydų reikalus prieš valdžią. Žodžiu, jie buvo visų žydų bendruomenių centras. Tie žydų seimai dažniausiai rinkdavosi Brastoje, nes ten buvo seniausioji žydų bendruomenė.
  teiginio_tipas: faktas
  patikimumo_lygis: vidutinis
  patikimumo_saltinis: ai
  patikimumo_pagrindimas: "Citata tiesiogiai pagrindžia šiame įraše nurodytą praktiką arba jos kontekstą."
  pagrindžia:
    - t-001

## Ryšiai
- Šaltinis: [[A. Šapoka (red.), Lietuvos istorija (1936 m.)]]
