---
tipas: paprotys
pavadinimas: 'Žiemos šventadienių vakarinis giedojimas grįčioje'
sukurta: ''
atnaujinta: ''
variantai: []
aliases: []
saltiniai:
  - 'Mūsų tautosaka, t.4 (1931)'
laikotarpis: ''
datos: []
periodas: ''
periodo_grupes: []
amziai: []
date_start: ''
date_end: ''
vaidmenys: []
dalyviai: []
susije_irasai: []
patikimumas: ''
tags:
  - paprotys
  - religinė-praktika
media_total_count: '0'
media_primary_thumb_url: ''
media_primary_canonical_url: ''
media_primary_directness: ''
media_primary_relation_type: ''
media_primary_json: ''
media_direct_json: |-
  []
media_contextual_json: |-
  []
media_all_json: |-
  []
---
# Žiemos šventadienių vakarinis giedojimas grįčioje

## Santrauka

Žiemos šventadieniais kaimo žmonės rinkdavosi į grįčią giedoti, o neatėjęs rytą ražončiaus giedoti galėjo būti išjuoktas ar išstumtas iš grįčios.

## Laikotarpis ir datos
Nenurodyta

## Kas tai
- kategorija: religinė praktika

## Atlikimas
- kas atlieka: kaimo jaunimas ir nejaunimas
- kada atliekama: žiemos šventadieniais, kai temsta
- kaip atliekama: surenkama viena grįčia vakaro giedojimui, o rytinio giedojimo vengiantysis baudžiamas pajuoka ar išstūmimu

## Paskirtis
Nenurodyta

## Kontekstas
Nenurodyta

## Teiginiai

<a id="claim-t-195392"></a>
- t-001
  global_id: t-195392
  teiginys: "Vakarinis giedojimas buvo patogus šventvakario užsiėmimas."
  atnaujinta: "2026-07-12 02:18"
  sprendimo_priezastis: "final::darbas/prompts/03_extraction/06_extract_customs_notes.md"
  teiginio_tipas: "faktas"
  šaltinio_profilis: "šališkumas: unknown; atribucija: optional"
  pagrindžia:
    - c-177811

<a id="claim-t-195393"></a>
- t-002
  global_id: t-195393
  teiginys: "Žiemos šventadieniais sutemus kaimo jaunimas ir nejaunimas rinkdavosi į vieną grįčią giedoti."
  atnaujinta: "2026-07-12 02:18"
  sprendimo_priezastis: "final::darbas/prompts/03_extraction/06_extract_customs_notes.md"
  teiginio_tipas: "faktas"
  šaltinio_profilis: "šališkumas: unknown; atribucija: optional"
  pagrindžia:
    - c-177809

<a id="claim-t-195394"></a>
- t-003
  global_id: t-195394
  teiginys: "Neatėjęs rytą ražončiaus giedoti galėjo būti apkaltintas tinginyste ir išstumtas iš grįčios."
  atnaujinta: "2026-07-12 02:18"
  sprendimo_priezastis: "final::darbas/prompts/03_extraction/06_extract_customs_notes.md"
  teiginio_tipas: "faktas"
  šaltinio_profilis: "šališkumas: unknown; atribucija: optional"
  pagrindžia:
    - c-177810

## Reikšmingi paminėjimai
Nenurodyta

## Citatos

- id: c-177809
  autorius: "M. Katkus"
  šaltinis: "Mūsų tautosaka, t.4 (1931) — M. Katkus, „Balanos gadynė“"
  puslapiai: "p. 152 (PDF 150)"
  indeksas: "Mūsų tautosaka, t.4 (1931) — M. Katkus, „Balanos gadynė“, p. 152 (PDF 150)."
  citata_originali: |
    Arba: šiandien susitrauksi, kitą dieną atsigausi.
    Ką gi žmonės veikdavo žiemos šventadieniais? Kai temsta, kaimo
    jaunimas ir nejaunimas rinkdavosi į vieną grįčią karunkos giedoti.
  citata_rodoma: "Ką gi žmonės veikdavo žiemos šventadieniais? Kai temsta, kaimo jaunimas ir nejaunimas rinkdavosi į vieną grįčią karunkos giedoti."
  statusas: verified
  atnaujinta: "2026-07-12 02:18"
  teiginio_tipas: faktas
  patikimumo_lygis: vidutinis
  patikimumo_saltinis: ai
  pagrindzia:
    - t-195393

- id: c-177810
  autorius: "M. Katkus"
  šaltinis: "Mūsų tautosaka, t.4 (1931) — M. Katkus, „Balanos gadynė“"
  puslapiai: "p. 152 (PDF 150)"
  indeksas: "Mūsų tautosaka, t.4 (1931) — M. Katkus, „Balanos gadynė“, p. 152 (PDF 150)."
  citata_originali: |
    Taigi į vakarinį
    giedojimą eina net tas, kuris tingėjo rytą keltis. Čia jį pastebi ir pri­
    kaišioja jam tinginystę, o jei jis yra dar augąs, tai kiti jo amžininkai,
    pusberniukai išstumia jį iš grįčios. Tai pabauda už nekėlimą, už ne­
    atėjimą rytą ražončiaus giedoti.
  citata_rodoma: "Čia jį pastebi ir prikaišioja jam tinginystę, o jei jis yra dar augąs, tai kiti jo amžininkai, pusberniukai išstumia jį iš grįčios. Tai pabauda už nekėlimą, už neatėjimą rytą ražončiaus giedoti."
  statusas: verified
  atnaujinta: "2026-07-12 02:18"
  teiginio_tipas: faktas
  patikimumo_lygis: vidutinis
  patikimumo_saltinis: ai
  pagrindzia:
    - t-195394

- id: c-177811
  autorius: "M. Katkus"
  šaltinis: "Mūsų tautosaka, t.4 (1931) — M. Katkus, „Balanos gadynė“"
  puslapiai: "p. 152 (PDF 150)"
  indeksas: "Mūsų tautosaka, t.4 (1931) — M. Katkus, „Balanos gadynė“, p. 152 (PDF 150)."
  citata_originali: |
    Kai temsta, kaimo
    jaunimas ir nejaunimas rinkdavosi į vieną grįčią karunkos giedoti. Čia
    jau aprenkama kita grįčia, ne maršalkos, ne taip „šventablyva“ , bet
    grįčia, kurioje yra daugiau mergaičių, nes, pagiedojus karunką ir dar
    kokią giesmę, atsiranda griežikas su smuiku, žinoma, jei ne adven­
    tas. Vakarinį giedojimą lankyti nesunku:
    anksti kelti nereikia,
    ir
    šventvakarį nėr kur dėtis, namie sėdint nėr kas veikti.
  citata_rodoma: "Vakarinį giedojimą lankyti nesunku: anksti kelti nereikia, ir šventvakarį nėr kur dėtis, namie sėdint nėr kas veikti."
  statusas: verified
  atnaujinta: "2026-07-12 02:18"
  teiginio_tipas: faktas
  patikimumo_lygis: vidutinis
  patikimumo_saltinis: ai
  pagrindzia:
    - t-195392
