---
tipas: 'saltinis'
pavadinimas: 'Lietuvių liaudies dainų tyrinėjimai'
saltinio_tipas: 'mokslinis veikalas'
autorius: 'Augustas Robertas Niemi'
saltiniai:
  - 'Mūsų tautosaka, t. 4 (1931)'
datos:
  - '1913 m.'
date_start: '1913'
date_end: '1913'
amziai:
  - 'XX'
sukurta: ''
atnaujinta: ''
media_total_count: '0'
media_primary_thumb_url: ''
media_primary_canonical_url: ''
media_primary_directness: ''
media_primary_relation_type: ''
media_primary_json: ''
media_direct_json: |-
  []
media_contextual_json: |-
  []
media_all_json: |-
  []
tags:
  - saltinis
---
# Lietuvių liaudies dainų tyrinėjimai

## Santrauka

A. R. Niemio veikalas, siejamas su 1910-1912 m. darbu Lietuvių mokslo draugijos archyve; 1913 m. jis minimas kaip Tautosakos komisijos spausdinamas kun. A. Sabaliausko vertimas.

## Bibliografiniai duomenys

A. Sabaliausko straipsnyje „A. a. prof. A. R. Niemi“.

## Teiginiai

<a id="claim-t-195225"></a>
- t-001
  global_id: t-195225
  teiginys: "Veikalas „Lietuvių liaudies dainų tyrinėjimai“ siejamas su A. R. Niemio 1910-1912 m. atliktu Lietuvių mokslo draugijos archyvo dainų katalogavimu, vadovaujant J. Basanavičiui."
  atnaujinta: "2026-07-11 22:44"
  sprendimo_priezastis: "final::darbas/prompts/03_extraction/01_extract_sources_notes.md"
  teiginio_tipas: "faktas"
  šaltinio_profilis: "šališkumas: unknown; atribucija: optional"
  pagrindžia:
    - c-177593

<a id="claim-t-195226"></a>
- t-002
  global_id: t-195226
  teiginys: "1913 m. minima knyga „Lietuvių liaudies dainų tyrinėjimai“."
  atnaujinta: "2026-07-11 22:44"
  sprendimo_priezastis: "final::darbas/prompts/03_extraction/01_extract_sources_notes.md"
  teiginio_tipas: "faktas"
  šaltinio_profilis: "šališkumas: unknown; atribucija: optional"
  pagrindžia:
    - c-177592

<a id="claim-t-195227"></a>
- t-003
  global_id: t-195227
  teiginys: "Veikalas spausdinamas Tautosakos komisijos kun. A. Sabaliausko vertime."
  atnaujinta: "2026-07-11 22:44"
  sprendimo_priezastis: "final::darbas/prompts/03_extraction/01_extract_sources_notes.md"
  teiginio_tipas: "faktas"
  šaltinio_profilis: "šališkumas: unknown; atribucija: optional"
  pagrindžia:
    - c-177591

## Citatos

- id: c-177591
  autorius: "A. Sabaliauskas"
  šaltinis: "Mūsų tautosaka, t.4 (1931) — A. Sabaliauskas, „A. a. prof. A. R. Niemi“"
  puslapiai: "p. 6 (PDF 5)"
  indeksas: "Mūsų tautosaka, t.4 (1931) — A. Sabaliauskas, „A. a. prof. A. R. Niemi“, p. 6 (PDF 5)."
  citata_originali: |
    L. Vienas lietuviškas vaikų pasakojimas Suomijoj.
    Virittaja.
    f
     1913 L. Lietuvių liaudies dainų tyrinėjimai.
    Kun. A. Sabaliausko vertime spausdinamas Tautosakos Komisijos.
  citata_rodoma: |
    Kun. A. Sabaliausko vertime spausdinamas Tautosakos Komisijos.
  statusas: verified
  atnaujinta: "2026-07-11 22:44"
  teiginio_tipas: faktas
  patikimumo_lygis: vidutinis
  patikimumo_saltinis: ai
  pagrindzia:
    - t-195227
- id: c-177592
  autorius: "A. Sabaliauskas"
  šaltinis: "Mūsų tautosaka, t.4 (1931) — A. Sabaliauskas, „A. a. prof. A. R. Niemi“"
  puslapiai: "p. 6 (PDF 5)"
  indeksas: "Mūsų tautosaka, t.4 (1931) — A. Sabaliauskas, „A. a. prof. A. R. Niemi“, p. 6 (PDF 5)."
  citata_originali: |
    1913 L. Lietuvių liaudies dainų tyrinėjimai.
    Kun. A. Sabaliausko vertime spausdinamas Tautosakos Komisijos.
    L. Lietuvių vestuvių apeigos.
  citata_rodoma: |
    1913 L. Lietuvių liaudies dainų tyrinėjimai.
  statusas: verified
  atnaujinta: "2026-07-11 22:44"
  teiginio_tipas: faktas
  patikimumo_lygis: vidutinis
  patikimumo_saltinis: ai
  pagrindzia:
    - t-195226
- id: c-177593
  autorius: "A. Sabaliauskas"
  šaltinis: "Mūsų tautosaka, t.4 (1931) — A. Sabaliauskas, „A. a. prof. A. R. Niemi“"
  puslapiai: "p. 5 (PDF 4)"
  indeksas: "Mūsų tautosaka, t.4 (1931) — A. Sabaliauskas, „A. a. prof. A. R. Niemi“, p. 5 (PDF 4)."
  citata_originali: |
    Reikia pridurti, kad a. a. prof. Niemį buvo gerai apsipažinęs tarp
    kito ko su estų liaudies dainomis. Jis jų yra peržiūrėjęs apie 40 tūks­
    tančių variantų, ypač kada didžiausio estų liaudies dainų rinkėjo Hur-
    to monumentalis dainų rinkinys, kad nepražūtų,
    karo
    metu buvo
    atgabentas į Helsinkus ir padėtas į Suomių literatūros dr-jos archyvą;
    tada suomiai to milžiniško veikalo yra pasidarę sau nuorašą.
    Be to, velionis, 1910— 12 metais viešėdamas Lietuvoje ir, nau­
    dodamasis Lietuvių mokslo dr-jos archyve esama tautosaka, vadovau­
    jamas a. a. d-ro Basanavičiaus, sukatalogavo ten esamas dainas, kaip
    matyti iš jo, dabar leidžiamo, veikalo „Lietuvių liaudies dainų tyri­
    nėjimai“ .
  citata_rodoma: |
    Be to, velionis, 1910— 12 metais viešėdamas Lietuvoje ir, nau­
    dodamasis Lietuvių mokslo dr-jos archyve esama tautosaka, vadovau­
    jamas a. a. d-ro Basanavičiaus, sukatalogavo ten esamas dainas, kaip
    matyti iš jo, dabar leidžiamo, veikalo „Lietuvių liaudies dainų tyri­
    nėjimai“ .
  statusas: verified
  atnaujinta: "2026-07-11 22:44"
  teiginio_tipas: faktas
  patikimumo_lygis: vidutinis
  patikimumo_saltinis: ai
  pagrindzia:
    - t-195225