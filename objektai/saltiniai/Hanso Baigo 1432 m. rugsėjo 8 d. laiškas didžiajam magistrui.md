---
tipas: saltinis
pavadinimas: 'Hanso Baigo 1432 m. rugsėjo 8 d. laiškas didžiajam magistrui'
saltiniai:
  - 'Michał Baliński, Vilniaus miesto istorija (2007 m.)'
datos:
  - '1432 m.'
date_start: '1432'
date_end: ''
sukurta: ''
atnaujinta: ''
tags:
  - kunigaikštis
  - miestas
  - saltinis
  - tauta
amziai:
  - 'XV'
---
# Hanso Baigo 1432 m. rugsėjo 8 d. laiškas didžiajam magistrui

## Santrauka

18 Kryžiuočių knechtas Hansas Baigas 1432 metų rugsėjo 8 dieną iš Trakų rašo didžiajam magistrui, kad: „Žy­ gimantas, naujas Lietuvos didysis kunigaikštis, visiems, turintiems iš tėvų paveldėto turto, suteikia garan­ tijų pagal Magdeburgo teises, kaip ir.

## Teiginiai

<a id="claim-t-88424"></a>
- t-001
  global_id: t-88424
  teiginys: 'Hansas Baigas 1432 m. rugsėjo 8 d. iš Trakų rašė didžiajam magistrui apie Žygimanto teikiamas garantijas pagal Magdeburgo teises.'
  sudarymo_pagrindimas: 'Citata pagrindžia laiško autorių, datą, vietą, adresatą ir praneštą turinį.'
  susije_objektai: 'mentioned_person: [[objektai/asmenys/Žygimantas|Žygimantas]]; mentioned_place: Trakai; mentioned_group: [[objektai/grupes/Kryžiuočių ordinas|Kryžiuočių ordinas]]; mentioned_object: [[objektai/zodynas/knechtas|knechtas]]; mentioned_person: [[objektai/asmenys/Švitrigaila|Švitrigaila]]; mentioned_place: Lietuva'
  temporaliniai_duomenys: 'įvykio data: 1432 m.'
  temporalinis_paaiskinimas: 'Ši data taikoma teiginyje minimai reikšmei „įvykio data“, o ne visam objekto laikotarpiui.'
  temporalinis_llm_pakomentavimas: 'Citata pagrindžia laiško autorių, datą, vietą, adresatą ir praneštą turinį.'
  pagrindžia:
    - c-001
  irodymo_stiprumas: 0.00
  saltinio_vieta: 441988-442505; hash=16784402f0423fb3fec10d15aaf072ef88d31536d4bccb829bbf13c4c0e7b412; match=whitespace_regex
  sprendimo_priezastis: auto
  ryšio_patikimumas: susije_su -> Trakai: 0.85
  ryšio_patikimumo_lygis: vidutinis
  ryšio_patikimumo_priezastys: owner_before_predicate; single_candidate_target; single_candidate_actor; target_after_predicate; same_sentence_locality
  ryšio_sprendimo_taisykle: rule_plain_mention
  ryšio_subjekto_parinkimas: Hanso Baigo 1432 m. rugsėjo 8 d. laiškas didžiajam magistrui: owner_note_path, source, gap=0
  ryšio_targeto_parinkimas: Trakai: mention_match, place, gap=38
  ryšio_paaiskinimas: Ryšys sukurtas taisykle "rule_plain_mention". Subjektas "Hanso Baigo 1432 m. rugsėjo 8 d. laiškas didžiajam magistrui" parinktas kaip owner_note_path. Targetas "Trakai" parinktas kaip mention_match aplink predikatą "mention". Patikimumą lėmė: owner_before_predicate, single_candidate_target, single_candidate_actor, target_after_predicate, same_sentence_locality.

## Reikšmingi paminėjimai

- c-001
  santrauka: 'Hansas Baigas 1432 m. rugsėjo 8 d. iš Trakų rašė didžiajam magistrui apie Žygimanto teikiamas garantijas pagal Magdeburgo teises.'
  šaltinis: Michał Baliński, Vilniaus miesto istorija (2007 m.)
  citata_originali: |
    18
    Kryžiuočių knechtas Hansas Baigas
    1432 metų rugsėjo 8 dieną iš Trakų
    rašo didžiajam magistrui, kad: „Žy­
    gimantas, naujas Lietuvos didysis
    kunigaikštis, visiems, turintiems iš
    tėvų paveldėto turto, suteikia garan­
    tijų pagal Magdeburgo teises, kaip
    ir tiems, kuriems seniau jo brolis Vy­
    tautas paskyrė dovanojimus, ir taip
    pelno miestiečių palankumą. Visi
    kalba: „O, koks mums maloningas
    yra mūsų valdovas, Švitrigaila to ne­
    darė. Už mūsų Žygimantą esame
    pasirengę numirti/' Kotzebue
    Switrig., p . 8 6 .
  citata_rodoma: ''
  teiginio_tipas: faktas
  statusas: verified
  pagrindžia:
    - t-001
  irodymo_stiprumas: 0.00
  saltinio_vieta: 441988-442505; hash=16784402f0423fb3fec10d15aaf072ef88d31536d4bccb829bbf13c4c0e7b412; match=whitespace_regex
  sprendimo_priezastis: auto
  ryšio_patikimumas: susije_su -> Trakai: 0.85
  ryšio_patikimumo_lygis: vidutinis
  ryšio_patikimumo_priezastys: owner_before_predicate; single_candidate_target; single_candidate_actor; target_after_predicate; same_sentence_locality
  ryšio_sprendimo_taisykle: rule_plain_mention
  ryšio_subjekto_parinkimas: Hanso Baigo 1432 m. rugsėjo 8 d. laiškas didžiajam magistrui: owner_note_path, source, gap=0
  ryšio_targeto_parinkimas: Trakai: mention_match, place, gap=38
  ryšio_paaiskinimas: Ryšys sukurtas taisykle "rule_plain_mention". Subjektas "Hanso Baigo 1432 m. rugsėjo 8 d. laiškas didžiajam magistrui" parinktas kaip owner_note_path. Targetas "Trakai" parinktas kaip mention_match aplink predikatą "mention". Patikimumą lėmė: owner_before_predicate, single_candidate_target, single_candidate_actor, target_after_predicate, same_sentence_locality.
