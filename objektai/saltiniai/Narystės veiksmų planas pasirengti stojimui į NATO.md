---
tipas: saltinis
pavadinimas: 'Narystės veiksmų planas pasirengti stojimui į NATO'
saltiniai:
  - 'Alfonsas Eidintas, Alfredas Bumblauskas, Antanas Kulakauskas, Mindaugas Tamošaitis, Lietuvos istorija (2013 m.)'
datos:
  - '1994 m.'
  - '1999 m.'
date_start: '1994'
date_end: '1999'
sukurta: ''
atnaujinta: ''
tags:
  - saltinis
amziai:
  - 'XX'
---
# Narystės veiksmų planas pasirengti stojimui į NATO

## Santrauka

1999 m. balandį NATO šalių vadovų susitikime Vašingtone pripažinus Lietuvos pastangas ir pažangą siekiant narystės, patvirtintas Narystės veiksmų planas pasirengti stojimui į NATO.

## Teiginiai

<a id="claim-t-21212"></a>
- t-001
  global_id: t-21212
  teiginys: '1999 m. balandį NATO šalių vadovų susitikime Vašingtone pripažinus Lietuvos pastangas ir pažangą siekiant narystės, patvirtintas Narystės veiksmų planas pasirengti stojimui į NATO.'
  sudarymo_pagrindimas: 'Teiginys yra pilnas faktinis sakinys apie plano patvirtinimą.'
  susije_objektai: 'mentioned_group: [[objektai/grupes/NATO|NATO]]; mentioned_place: Lietuva; mentioned_place: Vašingtonas'
  temporaliniai_duomenys: 'įvykio data: 1994 m.; įvykio data: 1999 m.'
  temporalinis_paaiskinimas: 'Ši data taikoma teiginyje minimai reikšmei „įvykio data“, o ne visam objekto laikotarpiui.'
  temporalinis_llm_pakomentavimas: 'Teiginys yra pilnas faktinis sakinys apie plano patvirtinimą.'
  pagrindžia:
    - c-001
  irodymo_stiprumas: 0.00
  saltinio_vieta: hash=b678eb75c5cbd0eba8ab63d95400970ecfeda7bb45f85bfabb210d75bb6c62fd; match=fallback; occurrences=0
  sprendimo_priezastis: auto
  ryšio_patikimumas: susije_su -> Lietuva: 0.85
  ryšio_patikimumo_lygis: vidutinis
  ryšio_patikimumo_priezastys: owner_before_predicate; single_candidate_target; single_candidate_actor; target_after_predicate; same_sentence_locality
  ryšio_sprendimo_taisykle: rule_plain_mention
  ryšio_subjekto_parinkimas: Narystės veiksmų planas pasirengti stojimui į NATO: owner_note_path, source, gap=0
  ryšio_targeto_parinkimas: Lietuva: mention_match, place, gap=51
  ryšio_paaiskinimas: Ryšys sukurtas taisykle "rule_plain_mention". Subjektas "Narystės veiksmų planas pasirengti stojimui į NATO" parinktas kaip owner_note_path. Targetas "Lietuva" parinktas kaip mention_match aplink predikatą "mention". Patikimumą lėmė: owner_before_predicate, single_candidate_target, single_candidate_actor, target_after_predicate, same_sentence_locality.

## Reikšmingi paminėjimai

- c-001
  šaltinis: Alfonsas Eidintas, Alfredas Bumblauskas, Antanas Kulakauskas, Mindaugas Tamošaitis, Lietuvos istorija (2013 m.)
  citata_originali: |
    1994 m. sausį Lietuvos Res-
    publikos prezidentas A. Brazauskas nusiuntė NATO generaliniam sekre-
    toriui Manfredui Verneriui (Manfred Wörner) laišką, kad Lietuva pagei-
    dauja tapti šios organizacijos nare. Tų pačių metų sausį Lietuva prisijungė
    prie Partnerystės taikos labui iniciatyvos. 1999 m. balandį NATO šalių
    vadovų susitikime Vašingtone pripažinus Lietuvos pastangas ir pažangą
    siekiant narystės, patvirtintas Narystės veiksmų planas pasirengti stoji-
    mui į NATO.
  citata_rodoma: ''
  teiginio_tipas: faktas
  statusas: verified
  pagrindžia:
    - t-001
  irodymo_stiprumas: 0.00
  saltinio_vieta: hash=b678eb75c5cbd0eba8ab63d95400970ecfeda7bb45f85bfabb210d75bb6c62fd; match=fallback; occurrences=0
  sprendimo_priezastis: auto
  ryšio_patikimumas: susije_su -> Lietuva: 0.85
  ryšio_patikimumo_lygis: vidutinis
  ryšio_patikimumo_priezastys: owner_before_predicate; single_candidate_target; single_candidate_actor; target_after_predicate; same_sentence_locality
  ryšio_sprendimo_taisykle: rule_plain_mention
  ryšio_subjekto_parinkimas: Narystės veiksmų planas pasirengti stojimui į NATO: owner_note_path, source, gap=0
  ryšio_targeto_parinkimas: Lietuva: mention_match, place, gap=51
  ryšio_paaiskinimas: Ryšys sukurtas taisykle "rule_plain_mention". Subjektas "Narystės veiksmų planas pasirengti stojimui į NATO" parinktas kaip owner_note_path. Targetas "Lietuva" parinktas kaip mention_match aplink predikatą "mention". Patikimumą lėmė: owner_before_predicate, single_candidate_target, single_candidate_actor, target_after_predicate, same_sentence_locality.
