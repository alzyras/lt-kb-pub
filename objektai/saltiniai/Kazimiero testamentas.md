---
tipas: saltinis
pavadinimas: "Kazimiero testamentas"
sukurta: "2026-04-18"
variantai:
  - "testamentu"
  - "to testamento"
aliases:
  - "Kazimiero testamentas"
  - "testamentu"
  - "to testamento"
saltiniai:
  - "[[A. Šapoka (red.), Lietuvos istorija (1936 m.)]]"
zymos:
  - "dokumentas"
laikotarpis: "1492 m."
datos:
  - "1492 m."
periodas: ""
periodo_grupes: []
amziai:
  - "XV"
date_start: "1492"
date_end: "1492"
vaidmenys: []
dalyviai: []
susije_irasai: []
patikimumas: "vidutinis"
---
# Kazimiero testamentas

## Bibliografiniai duomenys

- Pavadinimas: Kazimiero testamentas.
- Formos šaltinyje: testamentu; to testamento.
- Data / laikotarpis: 1492 m.
- Šaltinio rūšis / žymos: dokumentas.

## Turinys ir apimtis

Šaltinyje „Kazimiero testamentas“ pasakojama, kad Kazimieras esą testamentu padalijo Lietuvą ir Lenkiją, bet pats autorius pabrėžia, jog tokio dokumento istorikai nėra matę.

## Ryšiai

- Šaltinis: [[A. Šapoka (red.), Lietuvos istorija (1936 m.)]]

## Teiginiai

- id: t-001
  teiginys: "Šaltinyje „Kazimiero testamentas“ pasakojama, kad Kazimieras esą testamentu padalijo Lietuvą ir Lenkiją, bet pats autorius pabrėžia, jog tokio dokumento istorikai nėra matę."
  pagrindžia:
    - q-001
  šaltinis: [[A. Šapoka (red.), Lietuvos istorija (1936 m.)]]
  statusas: patvirtinta

## Reikšmingi paminėjimai

- id: q-001
  santrauka: Tekste pasakojama, kad Kazimieras esą testamentu padalijo Lietuvą ir Lenkiją, bet pats autorius pabrėžia, jog tokio dokumento istorikai nėra matę.
  šaltinis: [[A. Šapoka (red.), Lietuvos istorija (1936 m.)]]
  citata_originali: |
    Didysis kunigaikštis Kazimieras mirė Gardine, esą, Jonui Al-
    brechtui ir Aleksandrui testamentu padalinęs Lietuvą ir Lenkiją. Bet
    iš tikro to testamento niekas iš istorikų niekur nėra matęs.
  citata_rodoma: |
    Didysis kunigaikštis Kazimieras mirė Gardine, esą, Jonui Albrechtui ir Aleksandrui testamentu padalinęs Lietuvą ir Lenkiją. Bet iš tikro to testamento niekas iš istorikų niekur nėra matęs.
  teiginio_tipas: faktas
  patikimumo_lygis: vidutinis
  patikimumo_saltinis: ai
  patikimumo_pagrindimas: "Citata tiesiogiai įvardija arba aprašo šaltinį."
  pagrindžia:
    - t-001
