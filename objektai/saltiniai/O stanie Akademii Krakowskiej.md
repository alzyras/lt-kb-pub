---
tipas: saltinis
pavadinimas: 'O stanie Akademii Krakowskiej'
saltiniai:
  - 'Michał Baliński, Vilniaus miesto istorija (2007 m.)'
sukurta: ''
atnaujinta: ''
tags:
  - saltinis
  - vyskupas
---
# O stanie Akademii Krakowskiej

## Santrauka

Soltykovičius (O stanie Akademii Krakowskiej, p. 115) rašo, kad „Mikalojus iš Gožkovo, nuo Akademijos perkėlimo į Kroku­ vą Kazimiero laikais, buvo trečiasis jos rektorius ir taip pat trečiasis nuo Vilniaus katedros įsteigimo Lietuvos vvskupas".

## Teiginiai

<a id="claim-t-88458"></a>
- t-001
  global_id: t-88458
  teiginys: 'O stanie Akademii Krakowskiej nurodoma, kad Mikalojus iš Gožkovo buvo trečiasis Krokuvos akademijos rektorius.'
  sudarymo_pagrindimas: 'Teiginys yra aiškus, gramatiškas ir atitinka citatoje pateiktą Soltykovičiaus informaciją.'
  susije_objektai: 'mentioned_place: Gožkovas; mentioned_place: Krokuva; mentioned_place: Lietuva; mentioned_place: Vilnius'
  pagrindžia:
    - c-001
  irodymo_stiprumas: 0.00
  saltinio_vieta: hash=c1ba71782f0fbd99898a9cd94b74a422328d5a22ee93b7910ece13a93942e5f5; match=fallback; occurrences=0
  sprendimo_priezastis: auto
  ryšio_patikimumas: susije_su -> Gožkovas: 0.85
  ryšio_patikimumo_lygis: vidutinis
  ryšio_patikimumo_priezastys: owner_before_predicate; single_candidate_target; single_candidate_actor; target_after_predicate; same_sentence_locality
  ryšio_sprendimo_taisykle: rule_plain_mention
  ryšio_subjekto_parinkimas: O stanie Akademii Krakowskiej: owner_note_path, source, gap=0
  ryšio_targeto_parinkimas: Gožkovas: mention_match, place, gap=57
  ryšio_paaiskinimas: Ryšys sukurtas taisykle "rule_plain_mention". Subjektas "O stanie Akademii Krakowskiej" parinktas kaip owner_note_path. Targetas "Gožkovas" parinktas kaip mention_match aplink predikatą "mention". Patikimumą lėmė: owner_before_predicate, single_candidate_target, single_candidate_actor, target_after_predicate, same_sentence_locality.

## Reikšmingi paminėjimai

- c-001
  santrauka: 'O stanie Akademii Krakowskiej nurodoma, kad Mikalojus iš Gožkovo buvo trečiasis Krokuvos akademijos rektorius.'
  šaltinis: Michał Baliński, Vilniaus miesto istorija (2007 m.)
  citata_originali: |
    Ant­
    spaude turėjo Stumbro ženklą.
    ciškonų istorijoje T. VII, p. 157 pa­
    sakoja, kad 1399 metais Vilniaus
    vyskupu buvo pakeltas po Važilos
    pranciškonas Jonas Pliekta.
    5
    Kojelavičius ir Nar a movs-
    k i s klysta tą vyskupą Mikalojų va­
    dindami Borkovskiu, tai buvo Gož­
    kovskis, lenkas. Soltykovičius
    (O stanie Akademii Krakowskiej, p. 115)
    rašo, kad „Mikalojus iš Gožkovo,
    nuo Akademijos perkėlimo į Kroku­
    vą Kazimiero laikais, buvo trečiasis
    jos rektorius ir taip pat trečiasis nuo
    Vilniaus katedros įsteigimo Lietuvos
    vvskupas".
  citata_rodoma: ''
  teiginio_tipas: faktas
  statusas: verified
  pagrindžia:
    - t-001
  irodymo_stiprumas: 0.00
  saltinio_vieta: hash=c1ba71782f0fbd99898a9cd94b74a422328d5a22ee93b7910ece13a93942e5f5; match=fallback; occurrences=0
  sprendimo_priezastis: auto
  ryšio_patikimumas: susije_su -> Gožkovas: 0.85
  ryšio_patikimumo_lygis: vidutinis
  ryšio_patikimumo_priezastys: owner_before_predicate; single_candidate_target; single_candidate_actor; target_after_predicate; same_sentence_locality
  ryšio_sprendimo_taisykle: rule_plain_mention
  ryšio_subjekto_parinkimas: O stanie Akademii Krakowskiej: owner_note_path, source, gap=0
  ryšio_targeto_parinkimas: Gožkovas: mention_match, place, gap=57
  ryšio_paaiskinimas: Ryšys sukurtas taisykle "rule_plain_mention". Subjektas "O stanie Akademii Krakowskiej" parinktas kaip owner_note_path. Targetas "Gožkovas" parinktas kaip mention_match aplink predikatą "mention". Patikimumą lėmė: owner_before_predicate, single_candidate_target, single_candidate_actor, target_after_predicate, same_sentence_locality.
