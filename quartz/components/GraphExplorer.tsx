  ...
  const GraphExplorer: QuartzComponent = () => (
    <main class="graph-explorer" data-graph-explorer data-panel="hidden">
      <header class="graph-explorer-toolbar">
        ...
        <div class="graph-explorer-toolbar-actions" role="toolbar" aria-label="Žemėlapio filtrai">
          ...
          <button
            type="button"
            data-panel-toggle
            aria-label="Rodyti arba slėpti panelį"
            title="Detalės"
          >
            <Icon>
              <path d="M4 4h7v16H4zM15 5h5M15 12h5M15 19h5" />
            </Icon>
          </button>
          <button
            type="button"
        ...
        <div class="graph-explorer-popovers">
          <section class="graph-explorer-popover" data-popover-panel="types" hidden>
            <div class="graph-explorer-popover-header">
              <strong>Objektų tipai</strong>
              <button type="button" data-popover-close aria-label="Uždaryti">
                ×
              </button>
            </div>
            <div class="graph-explorer-check-grid" data-type-list></div>
            <label class="graph-explorer-isolated-toggle">
              <input type="checkbox" name="showIsolated" /> <span>Rodyti nesusietus objektus</span>
              <b data-isolated-count></b>
            </label>
          </section>

          <section
            class="graph-explorer-popover graph-explorer-relations-popover"
            data-popover-panel="relations"
            hidden
          >
            <div class="graph-explorer-popover-header">
              <strong>Ryšių tipai</strong>
              <button type="button" data-popover-close aria-label="Uždaryti">
                ×
              </button>
            </div>
          ...
          <section class="graph-explorer-popover" data-popover-panel="books" hidden>
            ...
            <div class="graph-explorer-source-actions">
              <button type="button" data-source-select-all>
                Visos
              </button>
              <button type="button" data-source-clear>
                Nuimti
              </button>
            </div>
            <div class="graph-explorer-source-list" data-source-list>
              <p>Kraunamos knygos…</p>
            </div>
          </section>

          <section class="graph-explorer-popover" data-popover-panel="filters" hidden>
            <div class="graph-explorer-popover-header">
              <strong>Daugiau filtrų</strong>
              <button type="button" data-popover-close aria-label="Uždaryti">
                ×
              </button>
            </div>
            <div class="graph-explorer-filter-grid">
              <label>
                ...
                <input type="number" name="minClaims" min="0" max="500" step="1" />
              </label>
              <label>
                <span>Min. citatų</span>
                <input type="number" name="minQuotes" min="0" max="500" step="1" />
              </label>
              <label>
                <span>Min. patikimumas</span>
                <input type="range" name="minConfidence" min="0" max="1" step="0.05" />
                <output data-confidence-output></output>
              </label>
              <label>
                <span>Kryptis</span>
                <select name="direction">
                  <option value="both">Abi kryptys</option>
                  <option value="out">Išeinantys</option>
                  <option value="in">Įeinantys</option>
                </select>
              </label>
              <label>
                <span>Nuo</span>
      ...
      <section class="graph-explorer-context" data-focus-context hidden>
        <div class="graph-explorer-focus-title">
          <span>Pasirinkta</span>
          <strong data-focus-title></strong>
          <button type="button" data-clear-focus aria-label="Uždaryti fokusą">
            ×
          </button>
        </div>
      ...
      <section class="graph-explorer-stage">
        ...
        <div class="graph-explorer-canvas" data-graph-canvas>
          <div class="graph-explorer-status" data-graph-status>
            Kraunamas žemėlapis…
          </div>
