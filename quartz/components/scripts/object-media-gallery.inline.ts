...
function galleryUrl(state: GalleryState, mediaId = "", galleryPath = location.pathname): string {
  return `${galleryPath}${serializeGalleryState(state, mediaId)}${location.hash}`
}
...
function writeUrl(
  ...
  galleryPath = location.pathname,
) {
  history[mode === "push" ? "pushState" : "replaceState"](
    {},
    "",
    galleryUrl(state, mediaId, galleryPath),
  )
}
...
function card(entry: MediaEntry, index: number): HTMLElement {
  ...
  const objects = (entry.relatedObjects ?? [])
    ...
    .join(" · ")
  const href = mediaDetailUrl(entry)
...
function initViewer(
    ...
    lightbox.pswp?.ui?.registerElement({
      ...
      onInit: (element, pswp) => {
            ...
            ?.addEventListener("click", async () => {
              ...
              try {
                await navigator.clipboard.writeText(
                  new URL(mediaDetailUrl(detail), location.origin).toString(),
                )
        ...
        pswp.on("change", () => {
          void update()
          const entry = getEntries()[pswp.currIndex]
          if (entry && openedWithPush) history.replaceState({}, "", mediaDetailUrl(entry))
        })
        void update()
      },
    })
  ...
  lightbox.on("close", () => {
    ...
    if (openedWithPush && location.pathname !== galleryPath) {
      openedWithPush = false
      history.back()
...
function initGallery(root: HTMLElement) {
  ...
  const galleryPath = root.dataset.galleryPath || location.pathname
