import { objectMapPreviewNeighbours } from "../../util/objectMapPreview"

// Inline scripts are bundled together with third-party gallery code. Keep the
// preview implementation in its own lexical scope so minified names from a
// dependency cannot overwrite one of its initialisers (which otherwise makes
// the SPA router fail before it is registered).
;(function isolateObjectMapPreview() {
  ...
  const objectMapVisual = objectMapRuntime.__ltkbGraphVisualRegistry ?? {
    // The standalone/local preview does not go through renderPage.tsx, which
    // normally injects the shared graph registry. Keep its palette in sync here
    // so object maps always preserve the object-type distinction.
    typeColors: {
      asmuo: 0x286456,
      autorius: 0x5d6f63,
      ivykis: 0x923120,
  ...
  function objectMapTopologyIndex(
    ...
    const direct = nodes.find((node) => node.slug === requestedSlug)
    const stripIndex = (value: string) => (value.endsWith("/index") ? value.slice(0, -6) : value)
    const normalized = stripIndex(requestedSlug).normalize("NFC").toLocaleLowerCase("lt")
    const matches = nodes.filter(
      (node) => stripIndex(node.slug).normalize("NFC").toLocaleLowerCase("lt") === normalized,
    )
    const focus = direct ?? (matches.length === 1 ? matches[0] : undefined)
    if (!focus) throw new Error("Object is missing from the full graph topology")
    ...
    const focusNode: ObjectMapPreviewNode = {
      ...focus,
      links,
      totalRelationCount: links.length,
    }
    const index: Record<string, ObjectMapPreviewNode> = { [focus.slug]: focusNode }
    for (const link of links) {
      if (!link.target || index[link.target]) continue
      index[link.target] = {
        slug: link.target,
  ...
  async function loadObjectMapIndex(slug: string): Promise<Record<string, ObjectMapPreviewNode>> {
    try {
      const topology = objectMapRuntime.loadGraphTopology
        ? await objectMapRuntime.loadGraphTopology()
        : await fetch("/static/graph-data/topology.json", { cache: "no-cache" }).then(
            (response) => {
              if (!response.ok) throw new Error("Failed to fetch graph topology")
              return response.json()
            },
          )
      return objectMapTopologyIndex(topology, slug)
    } catch {
      // A small object shard keeps the page usable while a local/incremental topology is absent.
    }
    let hash = 2166136261
  ...
  async function renderObjectMapPreview(root: HTMLElement) {
    ...
    try {
      ...
      const semanticCount = Number(node.totalRelationCount ?? root.dataset.objectSemanticCount ?? 0)
      const total = neighbours.length
      if (count) {
        count.textContent = `${total.toLocaleString("lt-LT")} kaimynai · ${semanticCount} ryšiai`
      }
