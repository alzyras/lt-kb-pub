...
for (const [i,[name, pattern, focus]] of families.entries()) {
  ...
  const status=i<3?"Parengta paroda, straipsnis ir šaltiniais paremtas narių registras; aprėptis apibrėžta pasirinktais genealoginiais šaltiniais":"Suplanuota; surinkti vietiniai kandidatai"
  ...
  rows.push(`| ${i+1} | [${name}](${id}.md) | ${stage} | ${matches.length} | ${focus} | ${i<3?"Paroda ir narių registras parengti":"Laukia tyrimo"} |`)
}
