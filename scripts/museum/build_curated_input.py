...
commons=json.load(open('/tmp/museum-commons.json'))['query']['pages'];bytitle={p['title'].replace('_',' '):p for p in commons.values() if p.get('imageinfo')}
...
def media_record(p,key,caption,date,portrait=True):
 ...
 if not src.exists():raise ValueError('Missing inspected asset: '+str(src))
 im=Image.open(src).convert('RGB');im.thumbnail((1000,1200) if portrait else (1000,750));dest=ROOT/f'quartz/static/media/museum/{key}.webp';dest.parent.mkdir(parents=True,exist_ok=True);im.save(dest,'WEBP',quality=88)
...
for key,date in zip(order,dates,strict=True):
 ...
 if key=='mindaugas':wiki['infobox'][1]['value']='Lietuvos didysis kunigaikštis; karalius nuo 1253 m.'
 if key=='ona-jogailaite':wiki['infobox'][1]['value']='Išrinktoji Lenkijos karalienė ir Lietuvos didžioji kunigaikštienė; bendravaldė su Steponu Batoru'
...
place_pages=json.load(open(ROOT/'scripts/museum/places-sources.json'))['query']['pages'];place_pages={p['title']:p for p in place_pages.values()};enpages=json.load(open('/tmp/museum-places-images.json'))['query']['pages'];enpages={p['title']:p for p in enpages.values()};pc=json.load(open('/tmp/museum-places-commons.json'))['query']['pages'];pcb={p['title'].replace('_',' '):p for p in pc.values()}
...
ex={'schemaVersion':'ltkb-exhibition/v1','exhibitionId':'ex-lietuvos-ir-atr-valdovai','slug':'parodos/lietuvos-ir-atr-valdovai','title':'Lietuvos ir ATR valdovai','subtitle':'Nuo Mindaugo iki Stanislovo Augusto Poniatovskio','description':'Septyni valstybingumo šimtmečiai, 33 valdovų istorijos. Nuo ankstyvosios Lietuvos kūrimosi ir dinastinių kovų iki renkamos Respublikos monarchijos, Apšvietos reformų ir 1795 metų. Atvaizdai, valdymo datos ir pasakojimai kviečia pažinti žmones, kurių sprendimai keitė valstybę.','layout':'chronological','status':'draft','noindex':True,'heroMediaId':rulers[-1]['media']['mediaId'],'sections':[]}
