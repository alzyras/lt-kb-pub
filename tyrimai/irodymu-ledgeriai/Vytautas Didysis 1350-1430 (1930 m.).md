---
tipas: tyrimas
pavadinimas: 'Įrodymų ledgerio suvestinė - Vytautas Didysis 1350-1430 (1930 m.)'
saltiniai:
  - 'Vytautas Didysis 1350-1430 (1930 m.)'
tags: []
---
# Įrodymų ledgerio suvestinė - Vytautas Didysis 1350-1430 (1930 m.)

## Vykdymo metaduomenys
- knyga: Vytautas Didysis 1350-1430 (1930 m.)
- viešasis šaltinis: Vytautas Didysis 1350-1430 (1930 m.)
- paskutinis_promptas: `darbas/prompts/05_quality_control/00_build_evidence_ledger.md`
- ivykdyti_promptai:
  - `darbas/prompts/00_common/01_rules.md`
  - `darbas/prompts/00_common/03_naming_and_note_style.md`
  - `darbas/prompts/00_common/04_citation_policy.md`
  - `darbas/prompts/00_common/05_linking_rules.md`
  - `darbas/prompts/00_common/06_quality_criteria.md`
  - `darbas/prompts/00_common/07_deduplication.md`
  - `darbas/prompts/00_common/09_evidence_ledger.md`
  - `darbas/prompts/00_common/10_scale_and_registry.md`
  - `darbas/prompts/00_common/11_claim_level_evidence.md`
  - `darbas/prompts/05_quality_control/00_build_evidence_ledger.md`

## Kandidatų mastas
- authors: 112
- customs: 49
- events: 101
- groups: 59
- items: 32
- people: 246
- places: 250
- sayings: 64
- sources: 111
- vocabulary: 85

## Kandidatų ledgerio būsenos
- authors: `verified 173`, `quote_not_found 178`, `no_quote 33`, `ambiguous 11`
- customs: `verified 78`, `quote_not_found 8`, `no_quote 63`, `ambiguous 2`
- events: `verified 206`, `quote_not_found 8`, `no_quote 91`, `ambiguous 0`
- groups: `verified 428`, `quote_not_found 16`, `no_quote 10`, `ambiguous 0`
- items: `verified 94`, `quote_not_found 14`, `no_quote 1`, `ambiguous 0`
- people: `verified 551`, `quote_not_found 1094`, `no_quote 0`, `ambiguous 0`
- places: `verified 1137`, `quote_not_found 38`, `no_quote 0`, `ambiguous 0`
- sayings: `verified 135`, `quote_not_found 9`, `no_quote 15`, `ambiguous 3`
- sources: `verified 320`, `quote_not_found 22`, `no_quote 2`, `ambiguous 0`
- vocabulary: `verified 241`, `quote_not_found 8`, `no_quote 0`, `ambiguous 2`

## Atrankos pastabos
- Ši suvestinė fiksuoja aukšto signalo, mechaniškai patvirtintų citatų branduolį ir kategorinę būsenų kontrolę po sujungtų kandidatų peržiūros.
- `quote_not_found`, `no_quote`, `ambiguous`, antraštiniai ar bibliografiniai vienetai nelaikomi galutine padengtimi ir negali blokuoti vėlesnio kūno citatos panaudojimo viešose pastabose.
- Didžiausios nebaigtų kandidatų sankaupos yra `people` ir `authors`, kur daug silpnų variantinių vardų, santrumpų ir OCR iškraipytų pėdsakų.
- `groups`, `places`, `sources`, `events` ir `vocabulary` turi didžiausią patvirtintų kūno citatų potencialą viešam integravimui.

## Patvirtintų citatų branduolys

## source-bibliografija-001
- status: unused
- claim_role: new_claim
- kategorija: saltinis
- kandidatas: Vytautas Didysis 1350-1430 (1930 m.)
- potencialus_teiginys: 1930 m. Kaune išleistas leidinys yra kolektyvinė knyga apie Vytautą Didįjį, pažymėta kaip „Sakalo“ bendrovės leidinys.
- public_note: objektai/saltiniai/Vytautas Didysis 1350-1430 (1930 m.).md
- saltinis: Vytautas Didysis 1350-1430 (1930 m.)
- vieta_saltinyje: pradžios puslapiai; match: exact
- santrauka: Tituliniame bloke vienu metu matyti autoriai, redaktorius, leidimo vieta, metai ir leidėjas.
- pagrindzia:
- pastaba: Aukšto signalo bibliografinis branduolys, tinkamas viešo šaltinio lapo papildomam stiprinimui.
  citata_originali: |
    VYTAUTAS DIDYSIS

    ## 1350 -

    ```
    Parašė:
    ```
    ##### A.Šapoka, Z. Ivinskis, V. Dėdinas, A. Kučinskas,

    ##### S. Sužiedėlis, P. Šležas, Dr. M. Krasauskaitė.

    ##### Redagavo P. Sležas

    ##### KAUNAS,

    ```
    1930 — VYTAUTO DIDŽIOJO — METAI
    ```
    ###### „SAKALO“ BENDROVĖS LEIDINYS


    Akcinė „Varpo“ B-vės spaustuvė, Kaunas, Gedimino gatvė Nr. 38.

## source-turinys-001
- status: unused
- claim_role: new_claim
- kategorija: saltinis
- kandidatas: Vytautas Didysis 1350-1430 (1930 m.)
- potencialus_teiginys: Turinys aiškiai rodo, kad leidinys aprėpia Vytauto jaunystę, politiką, Žalgirio mūšį, kristianizaciją, santykius su Lenkija, ekonominę politiką ir charakteristiką.
- public_note: objektai/saltiniai/Vytautas Didysis 1350-1430 (1930 m.).md
- saltinis: Vytautas Didysis 1350-1430 (1930 m.)
- vieta_saltinyje: turinio blokas; match: exact
- santrauka: Turinio citata apibrėžia knygos teminį stuburą ir autorių paskirstymą.
- pagrindzia:
- pastaba: Tinkama tiek bibliografinei apimčiai, tiek vėlesnei skyrių kilmės kontrolei.
  citata_originali: |
    A. Šapoka. Lietuva iki Vytauto ..................................................... XV
    Z. Ivinskis. Vytauto jaunystė ir jo veikimas iki 1392 m. 1
    V. Dėdinas. Vytauto vidaus ir užsienio politika ligi Žal­
    girio mūšio.....................................................................
    A. Kučinskas. Žalgirio mūšis .......................................................... 69
    S. Sužiedėlis. Vytauto vaidmuo Lietuvos kristianizacijoj 91
    S. Sužiedėlis. Vytauto galybės laikai .............................................
    P. Šležas. Vytauto santykiai su Lenkija ....................... 141
    P. Šležas. Vytauto konfliktas su Lenkija dėl karūnacijos ............. 205
    S.Sužiedėlis. Vytauto ekonominė politika ..................................... 235
    Dr. Krasauskaitė. Vytauto charakteristika ................................... 253
    A. Šapoka. Vytauto vieta mūsų istorijoj ....................................... 267

## people-vytautas-001
- status: unused
- claim_role: new_claim
- kategorija: asmuo
- kandidatas: Vytautas Didysis
- potencialus_teiginys: Vytautas pristatomas kaip žymiausias dinastijos atstovas, Kęstučio ir Birutės sūnus, gimęs apie 1350 m.
- public_note: objektai/asmenys/Vytautas.md
- saltinis: Vytautas Didysis 1350-1430 (1930 m.)
- vieta_saltinyje: chunk_0001; match: exact
- santrauka: Tiesioginė tapatybės ir datavimo citata pagrindiniam leidinio veikėjui.
- pagrindzia:
- pastaba: Tinkama esamo Vytauto asmens lapo papildymui kaip bazinis biografinis blokas.
  citata_originali: |
    Vytautas buvo Kęstučio iš eilės ketvirtas, o Birutės bene
    pirmagimis sūnus. Kuriais metais jis gimė, tiksliai sunku pasa­
    kyti. Konradas Bitschinas savo kronikoje sako, kad Vytautas,
    turėdamas dvidešimt metų, dalyvavęs 1370 metais Rudavos mū­
    šyje^2 ), 1430 metais mirdamas jis turėjęs aštuoniasdešimt metų,
    vadinasi turėjo gimti apie 1350 metus^3 ).

## people-vytautas-002
- status: unused
- claim_role: new_claim
- kategorija: asmuo
- kandidatas: Vytautas Didysis
- potencialus_teiginys: Vytautas aprašomas kaip neaukšto ūgio, energingas, pasitikintis savimi ir labai darbštus valdovas.
- public_note: objektai/asmenys/Vytautas.md
- saltinis: Vytautas Didysis 1350-1430 (1930 m.)
- vieta_saltinyje: chunk_0016; match: exact
- santrauka: Aukšto signalo charakteristikos citata apie išvaizdą, laikyseną ir darbo būdą.
- pagrindzia:
- pastaba: Tinka `Savybės` skyriui kaip stiprus kūno teksto liudijimas, ne vien vertinamoji antraštė.
  citata_originali: |
    Kaip užsilikusios žinios sako, Vytautas buvo neaukšto ūgio,
    dailių ir energingų veido bruožų. Turėjo dideles, gyvas ir pro­
    tingas akis. Iš veido atrodė moteriškas; nieks nepažino, kai jis
    apsitaisęs savo žmonos palydovės rūbais pabėgo iš kalėjimo.
    Visa Vytauto figūra atsišvietė pasitikėjimu savim, energija, gy­
    vumu, drąsa, rizikingumu ir savigarba, žodžiu, — kažkokia di­


    ```
    256
    ```
    dingo valdovo šviesa.

## customs-laidotuves-ir-velines-001
- status: unused
- claim_role: new_claim
- kategorija: paprotys
- kandidatas: Aisčių laidotuvių ir vėlinių apeigos
- potencialus_teiginys: Senovės lietuvių laidojimo ir vėlinių apeigose su mirusiuoju deginti jo daiktai, o vėlėms nuliedami gėrimai ir numetamas valgis.
- public_note:
- saltinis: Vytautas Didysis 1350-1430 (1930 m.)
- vieta_saltinyje: chunk_0001; match: exact
- santrauka: Viena citata tvirtai fiksuoja laidotuvių inventoriaus deginimą ir pomirtinio gyvenimo sampratą.
- pagrindzia:
- pastaba: Tinka papročio lapui kaip stiprus kūno įrodymas, neapsiribojantis vien terminine užuomina.
  citata_originali: |
    Dėl to buvo paprotys, kad mirus kilmingam žmo-
    gui (edelin) ginklus ir arklį, tarnus (knechte), moteris (meide),
    brangius apdarus, kurtus ir sakalus ir daug kitų padargų, rei-
    kalingų karo žygyje buvo sudeginama kartu su lavonu. Taip
    pat su paprasto žmogaus lavonu buvo deginama visa, kas
    priklausė jo užsiėmimui. Mat, visi tokie daiktai kartu su jais,
    pagal jų tikėjimą, turėjo prisikelti iš numirusių ir tarnauti
    toliau, kaip anksčiau tarnavo”1 2).

## items-karuna-001
- status: unused
- claim_role: new_claim
- kategorija: daiktas
- kandidatas: Karūna
- potencialus_teiginys: Vytauto karūna šaltinyje veikia kaip realus valdžios regalas ir Lietuvos savarankiškumo klausimo ženklas.
- public_note:
- saltinis: Vytautas Didysis 1350-1430 (1930 m.)
- vieta_saltinyje: chunk_0013; match: exact
- santrauka: Citata aiškiai sujungia karūnos klausimą su Lietuvos laisvės ir vasalystės problema.
- pagrindzia:
- pastaba: Aukšto signalo daikto ir politinio simbolio sankirtos liudijimas.
  citata_originali: |
    Tuo tarpu žymiai paaštrėjo santykiai su Lenkija, mat, da-
    bar aiškiai imta statyti klausimas: ar Lietuva laisva, ar ne?
    Jei laisva, tai Vytautas turi teisę užsidėt karaliaus karūną; jei
    ne, tai jam ir toliau tenka pasilikti Lenkijos vasalu. Vytauto
    karūnacija Lenkijai kėlė didelį pavojų^1 ), nes per ją Lietuva ga-
    lėjo visiškai išsivaduoti iš Lenkijos, o Lenkija dėl to būtų nete-
    kusi Europoj tos galybės ir reikšmės, kurią turėjo susijungusi
    su Lietuva.

## vocabulary-lenas-001
- status: unused
- claim_role: new_claim
- kategorija: zodynas
- kandidatas: lėnas
- potencialus_teiginys: Lėnas čia reiškia feodalinę valdą, kurią Vytautas pasižada valdyti Ordino naudai.
- public_note: objektai/zodynas/lenas.md
- saltinis: Vytautas Didysis 1350-1430 (1930 m.)
- vieta_saltinyje: chunk_0002; match: exact
- santrauka: Tiesioginė termininė citata, aiškinanti lėno reikšmę politiniame kontekste.
- pagrindzia:
- pastaba: Tinkama žodyno lapo papildymui, nes terminas aiškiai funkcionuoja teisės ir priklausomybės prasme.
  citata_originali: |
    Vytautas, pereidamas į Ordino pusę ir priimdamas krikštą, pasižadėjo vi-
    są savo tėviškę valdyti, kaip Ordino lėną, jei tik šis jam padėtų ją atgauti^2 ).

## sayings-kariaujame-ne-su-kryzium-001
- status: unused
- claim_role: new_claim
- kategorija: posakis
- kandidatas: Mes kariaujame ne su kryžium, o su vokiečiais!
- public_note:
- saltinis: Vytautas Didysis 1350-1430 (1930 m.)
- vieta_saltinyje: chunk_0001; match: exact
- santrauka: Aukšto signalo posakis, turintis savarankišką istorinę ir retorinę vertę.
- pagrindzia:
- pastaba: Nors posakis priskirtas Gediminui, jo vertė čia yra savarankiškas šaltinio retorinis vienetas.
  citata_originali: |
    Gediminas pirmas iš lie-
    tuvių pareiškė pasauliui: „Mes kariaujame ne su kryžium, o su
    vokiečiais!“

## Neužbaigti mazgai
- `authors` kategorijoje daugiausia nebaigtų variantų kyla iš bibliografinių santrumpų, OCR iškraipytų formų ir silpnų alternatyvių vardo išplėtimų; jie negali būti laikomi padengtimi be tikslios kūno citatos.
- `people` kategorijoje `quote_not_found` sankaupa labai didelė, nes į sujungimą pateko daug vienvardžių, silpnų varianto formų ir to paties asmens kartotinių aptikimų iš skirtingų gabalų.
- `customs` ir `sayings` kategorijose dalis `no_quote` bei `ambiguous` atvejų yra antraštiniai, reklaminiai ar formuliniai pėdsakai, kurie turi likti tik kaip atradimo užuominos.
- `sources`, `groups`, `places`, `events` ir `vocabulary` turi pakankamai patvirtintų kūno citatų potencialą viešam integravimui.

## Teiginiai

## Reikšmingi paminėjimai
